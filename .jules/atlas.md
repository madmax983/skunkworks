...
## 2026-03-25 - [Nova Misc Operator Untangling]
**Tangle:** The Blob - `experiments/chimera-lang/src/vm/mod.rs` still contained many domain-specific core logic execution blocks (`exec_prion_op`, `exec_scavenge_op`, `exec_digest_op`, `exec_havoc_op`, `exec_char_op`, `exec_mutagen_op`, `exec_findall_op`) improperly placed inside the God Object.
**Blueprint:** Extracted these nova and havoc execution blocks from `vm/mod.rs` and relocated them to `exec_havoc_op` in `experiments/chimera-lang/src/vm/havoc.rs` and the other ops into a new dedicated `experiments/chimera-lang/src/vm/ops/nova_misc.rs` module. This further enforces strict domain boundaries, reduces the size of `mod.rs` by an additional ~500 lines, and ensures cohesive code grouping.
