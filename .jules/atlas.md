...
## 2026-05-11 - [Extracting Binary Op]
**Tangle:** The Leak / Shotgun Surgery - The math operator module (`experiments/chimera-lang/src/vm/ops/math.rs`) calls a `binary_op` function that was implemented on the `ChimeraVM` instance in `experiments/chimera-lang/src/vm/mod.rs`. This leaked low-level operation detail into the central `mod.rs` rather than being collocated with the math operations in `ops/math.rs`.
**Blueprint:** Moved the `binary_op` function from `experiments/chimera-lang/src/vm/mod.rs` into `experiments/chimera-lang/src/vm/ops/math.rs` as an associated function of `ChimeraVM`. This further reduces the size of `vm/mod.rs` and encapsulates math-related stack operations inside `ops/math.rs`.
