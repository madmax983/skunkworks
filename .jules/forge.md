...
**[Flattening Match Pyramids into Match Statements]**\n**Learning:** Long if-else-if chains that branch entirely on variants of the same enum (like `app_state.view_mode`) create rightward drift and unnecessary complexity.\n**Action:** Use idiomatic `match` statements or guard clauses (e.g., `let Some(...) = ... else { return; };`) to flatten these branches, making the conditions clean and mutually exclusive.
