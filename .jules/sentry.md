...
**2024-05-24 - [Audio Engine State Machine Testing in resonance-audio]**
**Learning:** Event loops using `crossbeam_channel` for Audio parameters (like `AudioCommand::Oscillate` or `AudioCommand::PaintMaterial`) require extensive testing for not just correct events, but invalid ones. Out-of-bounds coordinates passed to grid manipulations or listener movement were silently swallowed (correct behavior), but without tests ensuring this lack of panic, future refactors might inadvertently add an `unwrap()` or `expect()`.
**Action:** Wrote a comprehensive `test_all_audio_commands` that runs the engine through every state mutation, explicitly feeding it valid coordinates and out-of-bounds coords, ensuring safety checks hold true across the grid bounds.

**2024-05-24 - [Origami Kinematics Math Guards in origami]**
**Learning:** `calculate_vertical` contained complex float math where `sqrt()` operations had negative checks (`if h_sq_4 < 0.0` or `if l_y_sq < 0.0`). Through algebraic analysis, I verified these are mathematically impossible under the domain of valid floats with the `clamp(0.01, 1.0)` constraint. However, they serve as critical safety guards against floating point precision loss or `NaN` injection.
**Action:** Wrote tests specifically testing `NaN` injection and extreme floats (`f32::INFINITY`) to ensure these guard clauses act correctly and avoid program termination (panic via `sqrt()` on negatives). We must prioritize keeping these defensive clauses even if standard coverage metrics complain they are "unreachable".
