...
**Added `neuro-sim` Network API Boundaries and `current_decay` loop**
**Learning:** `Network`'s external inputs and synapses didn't explicitly guard against mismatched index boundaries or disconnected synapses. Testing these verifies that it safely soft-fails instead of panicking on malformed inputs. Additionally, Izhikevich's internal `update` substep loop utilizes a `current_decay.abs() > 1.0e-6` check which zeroes the decay parameter below that bound; testing this branch ensures `current_decay` properly zeroes out across various decay coefficients (positive, negative, crossover).
**Action:** When manually stepping or mutating network layers, test out-of-bounds input arrays and connections to non-existent internal IDs. Wrote several unit tests for different zeroing conditions inside numerical stability internal loops, ensuring accuracy.
