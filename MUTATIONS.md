...
- **Concept**: Entomological Code Rot. ChimeraVM agents navigate the continuous codebase graph generated by mnem-rot. They execute DNA to forage for high-entropy (rotting) nodes and "repair" them by spending energy.
- **Novel trait**: Entomological Code Rot. Swarms of cybernetic agents adapting their pathfinding to locate areas of maximum code decay, acting as an active "immune system" for the codebase.
- **Status**: experiments/mnem-chimera
- **Evaluation**: Success. Compiled. The agents successfully forage entropy within the TUI graph.
### chimera-pachinko
- **Parents**: experiments/locust-ddos + experiments/chimera-lang
- **Concept**: Genetic Pachinko. Packets are dropped into a pachinko board where the pins are ChimeraVM instances.
- **Novel trait**: The pins execute genetic code to dynamically alter their restitution and apply forces to the packets, attempting to sort or score them.
- **Status**: experiments/chimera-pachinko
- **Evaluation**: Success. Compiled. The pins evolve to guide packets into high-scoring zones.

### etymological-mycelium
- **Parents**: experiments/myco-transit + experiments/biomorphic-lexicon
- **Concept**: Pheromone-Guided Etymology. Slime mold agents navigate a 3D terrain generated from the Levenshtein distance of words.
- **Novel trait**: Word strings form physical structures that attract pheromones, visually representing etymological drift as fungal growth.
- **Status**: experiments/etymological-mycelium
- **Evaluation**: Success. Compiled. Visual etymological mapping successful.

### gaze-attractor
- **Parents**: experiments/bifurcation-crawler + experiments/gaze-attractor
- **Concept**: TUI Bifurcation Attractor.
- **Novel trait**: TUI Bifurcation Attractor.
...
### chimera-fluid
- **Parents**: experiments/ferrous-fluid + experiments/chimera-lang
- **Concept**: Genetic Magnetic Fluids.
- **Novel trait**: Magnetic Particles containing ChimeraVM brains. They can process magnetic inputs and output forces to their surroundings.
- **Status**: experiments/chimera-fluid
- **Evaluation**: Pardoned by The Reaper ☠️. The terminal compilation failure was resolved by initializing `evolution_config` with `None`, allowing the specimen to compile successfully.
