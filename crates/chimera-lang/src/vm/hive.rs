            ...
            if let Some(val) = vm.stack.pop() {
                if let Value::Int(port) = val {
                    ...
                    match UdpSocket::bind(format!("0.0.0.0:{}", port)) {
                        Ok(socket) => {
                            if let Err(e) = socket.set_nonblocking(true) {
                                vm.output.push(format!(
                                    "HIVE: Failed to set nonblocking on {}: {}",
                                    port, e
                                ));
                            } else {
                        ...
                        Err(e) => {
                            vm.output
                                .push(format!("HIVE: Bind error on port {}: {}", port, e));
                        }
                    }
                } else {
                ...
                vm.output
                    .push("Error: Stack underflow for HiveBind".to_string());
            }
        }
        OpCode::HiveSend => {
            // [value, ip_str, port]
            if vm.stack.len() >= 3 {
                ...
                if let (Value::Int(port), Value::Str(ip)) = (port_val, ip_val) {
                    ...
                    match UdpSocket::bind("0.0.0.0:0") {
                        Ok(socket) => match serde_json::to_string(&msg_val) {
                            Ok(json) => {
                                if let Err(e) = socket.send_to(json.as_bytes(), &target) {
                                    vm.output
                                        .push(format!("HIVE: Send error to {}: {}", target, e));
                                } else {
                        ...
                        Err(e) => {
                            vm.output
                                .push(format!("HIVE: Socket creation error: {}", e));
                        }
                    }
                } else {
                    vm.output
                        .push("Error: Invalid args for HiveSend".to_string());
                }
            } else {
                vm.output
                    .push("Error: Stack underflow for HiveSend".to_string());
            }
        }
        OpCode::HiveRecv => {
            if let Some(val) = vm.stack.pop() {
                        ...
                        match socket.recv_from(&mut buf) {
                            Ok((amt, src)) => {
                                ...
                                match serde_json::from_slice::<Value>(data) {
                                    ...
                                    Err(e) => {
                                        vm.output.push(format!(
                                            "HIVE: Deserialization error from {}: {}",
                                            src, e
                                        ));
                            ...
                            Err(e) => {
                                vm.output
                                    .push(format!("HIVE: Recv error on port {}: {}", port, e));
                                vm.stack.push(Value::Int(0));
                            }
                        }
                    } else {
                ...
                vm.output
                    .push("Error: Stack underflow for HiveRecv".to_string());
            }
        }
        OpCode::HiveClose => {
            if let Some(val) = vm.stack.pop() {
