...
fn test_solve_distance_nan_params() {
    ...
    let p1 = system
        .add_particle(hyper_system::Vec4::zero(), 1.0)
        .unwrap();
    let p2 = system
        .add_particle(hyper_system::Vec4::zero(), 1.0)
        .unwrap();
...
fn test_solve_pin() {
    ...
    let p1 = system
        ...
        .unwrap();
    system
        .add_pin_constraint(p1, hyper_system::Vec4::new(1.0, 1.0, 1.0, 1.0))
        .unwrap();
    system.step(0.1, 1, 0.99);
    assert_eq!(
        system.particles[p1].pos,
        hyper_system::Vec4::new(1.0, 1.0, 1.0, 1.0)
    );
}
