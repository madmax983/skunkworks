...
mod tests {
    ...
    // We can't use tempfile crate in dependencies unless we add it to dev-dependencies.
    // But this is a workspace, maybe it's available?
    // Let's check `Cargo.toml`.
    // Actually, I don't want to add extra deps if I can avoid it.
    // I'll test scanning the current file itself.

    #[test]
    fn test_analyze_file_self() {
        // Let's rely on creating a temp file in `std::env::temp_dir()`.

        let mut temp_path = std::env::temp_dir();
        temp_path.push("ant_colony_test.rs");
