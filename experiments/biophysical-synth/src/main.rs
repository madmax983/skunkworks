...
async fn main() -> anyhow::Result<()> {
    ...
    loop {
        ...
        let target_current = if is_key_down(KeyCode::A) { 2.0 }
        else if is_key_down(KeyCode::S) { 5.0 }
        else if is_key_down(KeyCode::D) { 10.0 }
        else if is_key_down(KeyCode::F) { 20.0 }
        else if is_key_down(KeyCode::G) { 35.0 } // Depolarization block territory?
        else if is_key_down(KeyCode::H) { 50.0 }
        else if is_key_down(KeyCode::J) { 100.0 }
        else { 0.0 };

        if (target_current - current_inj).abs() > 0.1 {
            current_inj = target_current;
            audio_sys.command_tx.push(Command::SetCurrent(current_inj)).ok();
        }
        ...
        if is_key_down(KeyCode::Up) {
            last_state.g_na += 1.0;
            audio_sys.command_tx.push(Command::SetGNa(last_state.g_na)).ok();
        }
        ...
        if is_key_down(KeyCode::Right) {
            last_state.g_k += 1.0;
            audio_sys.command_tx.push(Command::SetGK(last_state.g_k)).ok();
        }
        ...
        draw_bar(start_x + spacing*2.0, bar_base, last_state.n, "n (K)", BLUE);

        // Conductances (Normalized roughly for display)
        draw_bar(start_x + spacing*4.0, bar_base, last_state.g_na / 200.0, "G_Na", RED);
        draw_bar(start_x + spacing*5.0, bar_base, last_state.g_k / 100.0, "G_K", BLUE);

        // Current
        draw_bar(start_x + spacing*6.0, bar_base, current_inj / 100.0, "I_inj", WHITE);

        // Instructions
        draw_text("CONTROLS:", 10.0, screen_height() - 60.0, 20.0, LIGHTGRAY);
        draw_text("Hold A/S/D/F/G/H/J to inject Current (Notes)", 10.0, screen_height() - 40.0, 20.0, LIGHTGRAY);
        draw_text("Arrows: Tune G_Na / G_K (Timbre) | Space: Pluck | R: Reset", 10.0, screen_height() - 20.0, 20.0, LIGHTGRAY);
