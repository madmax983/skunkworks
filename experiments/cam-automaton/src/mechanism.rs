...
impl Mechanism {
    ...
    pub fn add_cam(world: &mut PhysicsWorld, shaft_handle: RigidBodyHandle, shape: SharedShape, offset: Vector2<f32>) {
        let collider = ColliderBuilder::new(shape)
            .position(Isometry::new(offset, 0.0))
            .friction(0.0)
            .build();

        world.collider_set.insert_with_parent(collider, shaft_handle, &mut world.rigid_body_set);
    }
    ...
    pub fn create_follower(world: &mut PhysicsWorld, x: f32, y: f32) -> RigidBodyHandle {
        ...
        let collider = ColliderBuilder::cuboid(0.2, 1.0)
            .friction(0.0)
            .build();
        ...
        world.impulse_joint_set.insert(anchor_handle, handle, joint, true);
