...
use rayon::prelude::*;
use ::rand::Rng; // Explicitly use rand crate trait
...
async fn main() {
    ...
    let mut real_system = match loader::load_dependencies() {
        Ok(sys) => {
            println!("Loaded {} nodes and {} links", sys.nodes.len(), sys.links.len());
            sys
        },
        Err(e) => {
    ...
    let mut ghosts: Vec<PendulumSystem> = (0..ghost_count).map(|_| {
    ...
    loop {
        if is_key_down(KeyCode::Right) { target.x += 10.0 / zoom; }
        if is_key_down(KeyCode::Left) { target.x -= 10.0 / zoom; }
        if is_key_down(KeyCode::Down) { target.y += 10.0 / zoom; }
        if is_key_down(KeyCode::Up) { target.y -= 10.0 / zoom; }
        ...
        if is_mouse_button_down(MouseButton::Left) {
            ...
            let world_mouse = (mouse_vec - Vec2::new(screen_width()/2.0, screen_height()/2.0)) / zoom + target;
        ...
        draw_text(&format!("Nodes: {}", real_system.nodes.len()), 20.0, 60.0, 20.0, GRAY);
        draw_text(&format!("Ghosts: {}", ghosts.len()), 20.0, 80.0, 20.0, Color::new(0.0, 1.0, 1.0, 1.0));
        draw_text("Arrows: Move | +/-: Zoom | Mouse: Drag | G: Toggle Ghosts", 20.0, screen_height() - 20.0, 20.0, GRAY);
