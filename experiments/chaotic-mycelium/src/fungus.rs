        ...
        for y in 0..self.height {
            for x in 0..self.width {
                ...
                if let Some(parent) = self.came_from[idx] {
                    ...
                    draw_line(p1.x, p1.y, p2.x, p2.y, 1.0 * (cell_size / 2.0).max(1.0), Color::new(1.0, 1.0, 1.0, 0.4));
                }
            }
        }
        ...
        if self.found {
           if let Some(target) = self.target {
               ...
               while let Some(parent) = self.came_from[(curr.y as usize) * self.width + (curr.x as usize)] {
                    ...
                    draw_line(p1.x, p1.y, p2.x, p2.y, thickness * cell_size.max(1.0), pulse_color);
                    curr = parent;
               }
           }
        }
