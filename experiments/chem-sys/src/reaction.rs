...
impl ChemicalSystem {
    ...
    pub fn add_chemical(&mut self, x: usize, y: usize, amount: f64) {
        if x < self.width && y < self.height {
            ...
            self.v[i] += amount;
            // Also reduce U slightly to jumpstart reaction?
            // Standard seeding usually just sets V. But maintaining mass helps?
            // Actually, usually we set V=1 and U=0.5 or something in a region.
            // But let's just add V for now.
        }
    }
