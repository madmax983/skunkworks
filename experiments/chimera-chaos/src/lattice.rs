...
impl Lattice {
    pub fn new(width: usize, height: usize) -> Self {
        ...
        // Gradient from 3.0 to 4.0 (Chaotic regime starts around 3.57)
        let r_map = (0..size).map(|i| {
            let x = (i % width) as f32 / width as f32;
            let y = (i / width) as f32 / height as f32;
            // Base gradient (diagonal)
            let base_r = 3.2 + (x * 0.4) + (y * 0.4);
            // Add some noise
    ...
    pub fn get_val(&self, x: usize, y: usize) -> f32 {
        self.cells[y * self.width + x]
    }

    pub fn get_r(&self, x: usize, y: usize) -> f32 {
        self.r_map[y * self.width + x]
    }

    pub fn set_r(&mut self, x: usize, y: usize, val: f32) {
        self.r_map[y * self.width + x] = val.clamp(0.0, 4.0);
    }
}
