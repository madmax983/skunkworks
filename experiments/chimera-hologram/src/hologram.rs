...
pub struct HolographicMemory {
    pub width: usize,
    pub height: usize,
    pub memory: Vec<Complex<f64>>, // Accumulated interference pattern (Frequency Domain? No, see below)
                                   // In digital holography, we usually record the hologram in the spatial domain (CCD).
