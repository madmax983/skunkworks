...
* Random Orca signal bursts disrupt the grid logic.
* Extreme genetic chaos is applied: genes within strands are randomly shuffled, and chaotic mutations (like `Glitch`, `BioHack`, or `Supernova`) are forcefully inserted into the genome.

Use at your own risk. The results are unpredictable and highly volatile.
...
| `E` | **Exists/Any** | Reads **West** (List), **North** (Pattern). Outputs `1` to **Self** if Pattern exists in List. |
| `Z` | **Zip/Hyper**| Reads **West** (List), **North** (Op), **East** (List). Element-wise operation, outputs to **Self**. |
