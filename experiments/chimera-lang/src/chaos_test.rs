...
mod tests {
    ...
    fn test_evolve_blinker() {
        ...
        let setup_genes = vec![
            ...
            Gene { op: OpCode::Push, args: vec![Nucleotide::Number(4)] }, // y
        ...
        for _ in 0..12 { vm.step(); }
        // Execute Evolve
        vm.step();

        // Check new state: Horizontal line (5,4), (5,5), (5,6)
        match vm.grid[5][4] { Value::Int(1) => {}, _ => panic!("(5,4) should be alive") }
        match vm.grid[5][5] { Value::Int(1) => {}, _ => panic!("(5,5) should be alive") }
        match vm.grid[5][6] { Value::Int(1) => {}, _ => panic!("(5,6) should be alive") }

        // Check old vertical tips died
        match vm.grid[4][5] { Value::Int(0) => {}, _ => panic!("(4,5) should be dead") }
        match vm.grid[6][5] { Value::Int(0) => {}, _ => panic!("(6,5) should be dead") }
    }
    ...
    fn test_scramble() {
        ...
        genes.push(Gene { op: OpCode::Scramble, args: vec![] });
