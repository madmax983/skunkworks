...
fn test_storm_and_aeolus() {
    ...
    // Check center
    assert_eq!(vm.moisture_grid[cy][cx], 100, "Storm failed to add moisture");
    // Check neighbor
    if cy > 0 {
...
fn test_process_fluid_advection() {
    ...
    let target_x = cx + 5;
    if target_x < 16 {
        assert!(vm.moisture_grid[cy][target_x] > 0, "Moisture should advect to target");
        assert!(vm.moisture_grid[cy][cx] < 1000, "Source should lose moisture");
    }
}
...
fn test_tsunami() {
    ...
    for i in 0..8 {
        if cx + i < 16 {
            assert_eq!(vm.wind_grid[cy][cx + i], (0, 10), "Wind not set at index {}", i);
            assert_eq!(vm.moisture_grid[cy][cx + i], 50, "Moisture not added at index {}", i);
        }
    }
}
