    ...
    for section in program.into_inner() {
        ...
        match inner_block.as_rule() {
            ...
            Rule::prologue_block => {
                ...
                genes.push(Gene::new(OpCode::PrologueMadScientist, vec![]));
            }
            Rule::lisp_block => {
            ...
            Rule::chaos_hologram_block => {
                for instr in inner_block.into_inner() {
                    genes.extend(compile_chaos_hologram_instr(instr)?);
                }
            }
            Rule::hologram_text_block => {
                for instr in inner_block.into_inner() {
                    genes.extend(compile_hologram_text_instr(instr)?);
                }
            }
            Rule::hologram_block => {
                for instr in inner_block.into_inner() {
            ...
            Rule::verge_block => {
                for instr in inner_block.into_inner() {
                    genes.extend(compile_verge_instr(instr)?);
                }
            }
            Rule::cymatic_ocean_block => {
                for instr in inner_block.into_inner() {
                    genes.extend(compile_cymatic_ocean_instr(instr)?);
                }
            }
            Rule::quantum_garden_block => {
                for instr in inner_block.into_inner() {
                    genes.extend(compile_quantum_garden_instr(instr)?);
                }
            }
            Rule::entropic_rain_block => {
                for instr in inner_block.into_inner() {
                    genes.extend(compile_entropic_rain_instr(instr)?);
                }
            }
            Rule::spectral_scribe_block => {
                for instr in inner_block.into_inner() {
            ...
            Rule::orca_weaver_block => {
                for inner_pair in inner_block.into_inner() {
                    let mut inner_genes = compile_orca_weaver_instr(inner_pair)?;
                    genes.append(&mut inner_genes);
                }
            }
            Rule::elektra_weaver_block => {
                for inner_pair in inner_block.into_inner() {
                    let mut inner_genes = compile_elektra_weaver_instr(inner_pair)?;
                    genes.append(&mut inner_genes);
                }
            }
...
fn compile_spectral_scribe_instr(pair: pest::iterators::Pair<Rule>) -> Result<Vec<Gene>> {
    let mut genes = Vec::new();
    let inner = pair
        .into_inner()
        .next()
    ...
    match inner.as_rule() {
        Rule::identifier => {
            let op = inner.as_str().to_ascii_lowercase();
            if op == "encode" {
                genes.push(Gene::new(OpCode::SpectralScribe, vec![]));
            } else if let Ok(opcode) = OpCode::from_str(&op) {
                genes.push(Gene::new(opcode, vec![]));
            } else {
                genes.push(Gene::new(OpCode::Push, vec![Nucleotide::String(op)]));
...
fn compile_entropic_rain_instr(pair: pest::iterators::Pair<Rule>) -> Result<Vec<Gene>> {
    let mut genes = Vec::new();
    let inner = pair
        .into_inner()
        .next()
    ...
    match inner.as_rule() {
        Rule::identifier => {
            let op = inner.as_str().to_ascii_lowercase();
            if op == "simulate" {
                genes.push(Gene::new(OpCode::EntropicRain, vec![]));
            } else if let Ok(opcode) = OpCode::from_str(&op) {
                genes.push(Gene::new(opcode, vec![]));
            } else {
                genes.push(Gene::new(OpCode::Push, vec![Nucleotide::String(op)]));
...
fn compile_hologram_text_instr(pair: pest::iterators::Pair<Rule>) -> Result<Vec<Gene>> {
    ...
    match inner.as_rule() {
        Rule::identifier => {
            let op = inner.as_str().to_ascii_lowercase();
            if op == "reconstruct" {
                genes.push(Gene::new(OpCode::HologramText, vec![]));
            } else if let Ok(opcode) = OpCode::from_str(&op) {
                genes.push(Gene::new(opcode, vec![]));
            } else {
                genes.push(Gene::new(OpCode::Push, vec![Nucleotide::String(op)]));
...
fn compile_quantum_garden_instr(pair: pest::iterators::Pair<Rule>) -> Result<Vec<Gene>> {
    ...
    match inner.as_rule() {
        Rule::identifier => {
            let op = inner.as_str().to_ascii_lowercase();
            if op == "simulate" {
                genes.push(Gene::new(OpCode::QuantumGarden, vec![]));
            } else if let Ok(opcode) = OpCode::from_str(&op) {
                genes.push(Gene::new(opcode, vec![]));
            } else {
                genes.push(Gene::new(OpCode::Push, vec![Nucleotide::String(op)]));
...
fn compile_cymatic_ocean_instr(pair: pest::iterators::Pair<Rule>) -> Result<Vec<Gene>> {
    ...
    match inner.as_rule() {
        Rule::identifier => {
            let op = inner.as_str().to_ascii_lowercase();
            if op == "simulate" {
                genes.push(Gene::new(OpCode::CymaticOcean, vec![]));
            } else if let Ok(opcode) = OpCode::from_str(&op) {
                genes.push(Gene::new(opcode, vec![]));
            } else {
                genes.push(Gene::new(OpCode::Push, vec![Nucleotide::String(op)]));
...
fn compile_verge_instr(pair: pest::iterators::Pair<Rule>) -> Result<Vec<Gene>> {
    ...
    match inner.as_rule() {
        Rule::identifier => {
            let op = inner.as_str().to_ascii_lowercase();
            if op == "simulate" {
                genes.push(Gene::new(OpCode::Verge, vec![]));
            } else if let Ok(opcode) = OpCode::from_str(&op) {
                genes.push(Gene::new(opcode, vec![]));
            } else {
                genes.push(Gene::new(OpCode::Push, vec![Nucleotide::String(op)]));
    ...
    match inner.as_rule() {
        Rule::identifier => {
            ...
            if op == "execute" || op == "run" {
                genes.push(Gene::new(OpCode::ChaosHologram, vec![]));
            } else if let Ok(opcode) = OpCode::from_str(&op) {
                genes.push(Gene::new(opcode, vec![]));
            } else {
                genes.push(Gene::new(OpCode::Push, vec![Nucleotide::String(op)]));
        ...
        _ => return Err(anyhow!("Unexpected token in chaos_hologram block")),
    }
...
fn compile_hologram_instr(pair: pest::iterators::Pair<Rule>) -> Result<Vec<Gene>> {
    ...
    match inner.as_rule() {
        Rule::identifier => {
            let op = inner.as_str().to_ascii_lowercase();
            if op == "simulate" {
                genes.push(Gene::new(OpCode::Hologram, vec![]));
            } else if let Ok(opcode) = OpCode::from_str(&op) {
                genes.push(Gene::new(opcode, vec![]));
            } else {
                genes.push(Gene::new(OpCode::Push, vec![Nucleotide::String(op)]));
...
"#;
    ...
    #[test]
    fn test_hologram_text_block() {
        let code = r#"
hologram_text {
    "Start hologram text sim"
    100
    reconstruct
}
"#;
...
fn compile_orca_weaver_instr(pair: pest::iterators::Pair<Rule>) -> Result<Vec<Gene>> {
    let mut genes = Vec::new();
    let inner = pair
        .into_inner()
        .next()
    ...
    match inner.as_rule() {
        Rule::identifier => {
            ...
            if op == "weave" || op == "simulate" {
                genes.push(Gene::new(OpCode::OrcaWeaver, vec![]));
            } else if let Ok(opcode) = OpCode::from_str(&op) {
                genes.push(Gene::new(opcode, vec![]));
            } else {
                genes.push(Gene::new(OpCode::Push, vec![Nucleotide::String(op)]));
...
fn compile_elektra_weaver_instr(pair: pest::iterators::Pair<Rule>) -> Result<Vec<Gene>> {
    let mut genes = Vec::new();
    let inner = pair
        .into_inner()
        .next()
    ...
    match inner.as_rule() {
        Rule::identifier => {
            let op = inner.as_str().to_ascii_lowercase();
            if op == "weave" || op == "simulate" {
                genes.push(Gene::new(OpCode::ElektraWeaver, vec![]));
            } else if let Ok(opcode) = OpCode::from_str(&op) {
                genes.push(Gene::new(opcode, vec![]));
            } else {
                genes.push(Gene::new(OpCode::Push, vec![Nucleotide::String(op)]));
...
#[cfg(test)]
mod tests_elektra_weaver {
    use super::*;
    #[test]
    fn test_elektra_weaver_block() {
        let source = r#"
        elektra_weaver {
            "Weaving elektra construct"
            100
            weave
        }
        "#;
        ...
        assert_eq!(
            genes[0].args[0],
            Nucleotide::String("Weaving elektra construct".to_string())
        );
        assert_eq!(genes[1].op, OpCode::Push);
        assert_eq!(genes[1].args[0], Nucleotide::Number(100));
