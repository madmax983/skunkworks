                                ...
                                ViewMode::Genome => {
                                    ...
                                    match ChimeraParser::parse(Rule::gene, &app_state.input_buffer)
                                    {
                                        Ok(mut pairs) => {
                                            ...
                                            {
                                                let helix =
                                                    &mut std::rc::Rc::make_mut(&mut vm.dna).helix;
                                                let strand = std::rc::Rc::make_mut(
                                                    &mut helix.strands[app_state.selected_strand],
                                                );
                                                strand.genes[app_state.selected_gene] = gene;
