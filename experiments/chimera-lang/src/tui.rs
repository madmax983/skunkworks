    ...
    loop {
        ...
        if event::poll(std::time::Duration::from_millis(100))? {
            if let Event::Key(key) = event::read()? {
                if !handle_input_event(key, vm, app_state)? {
                    return Ok(());
                }
            }
        }
...
fn handle_input_event(key: event::KeyEvent, vm: &mut ChimeraVM, app_state: &mut AppState) -> Result<bool> {
    #[cfg(feature = "nova")]
    if vm.spirit_request {
    ...
    if let InputMode::Injection = app_state.input_mode {
        return handle_injection_mode(key, vm, app_state);
    }

    if let InputMode::Editing = app_state.input_mode {
        return handle_editing_mode(key, vm, app_state);
    }

    handle_normal_mode(key, vm, app_state)
}

#[cfg(feature = "nova")]
fn handle_spirit_request(key: event::KeyEvent, vm: &mut ChimeraVM, app_state: &mut AppState) -> Result<bool> {
    match key.code {
        KeyCode::Enter => {
...
fn handle_injection_mode(key: event::KeyEvent, vm: &mut ChimeraVM, app_state: &mut AppState) -> Result<bool> {
    match key.code {
        KeyCode::Enter => {
...
fn handle_editing_mode(key: event::KeyEvent, vm: &mut ChimeraVM, app_state: &mut AppState) -> Result<bool> {
    match key.code {
        KeyCode::Enter => {
...
fn handle_normal_mode(key: event::KeyEvent, vm: &mut ChimeraVM, app_state: &mut AppState) -> Result<bool> {
    #[cfg(feature = "nova")]
    ...
    match key.code {
        KeyCode::Tab => handle_view_switching(app_state),
        KeyCode::Down | KeyCode::Up | KeyCode::Right | KeyCode::Left | KeyCode::Enter => {
            handle_navigation(key, vm, app_state);
        }
        _ => return handle_global_hotkeys(key, vm, app_state),
    }
    Ok(true)
}

fn handle_view_switching(app_state: &mut AppState) {
...
fn handle_global_hotkeys(key: event::KeyEvent, vm: &mut ChimeraVM, app_state: &mut AppState) -> Result<bool> {
    match key.code {
...
fn navigate_genome(key: event::KeyEvent, vm: &mut ChimeraVM, app_state: &mut AppState) {
    match key.code {
        KeyCode::Down => {
...
fn navigate_grid(key: event::KeyEvent, vm: &mut ChimeraVM, app_state: &mut AppState) {
    match key.code {
        KeyCode::Down => {
...
#[cfg(feature = "biophysics")]
fn navigate_cortex(key: event::KeyEvent, vm: &mut ChimeraVM, app_state: &mut AppState) {
    match key.code {
        KeyCode::Down => {
            let mut neurons_sorted: Vec<_> = vm.neurons.keys().collect();
            neurons_sorted.sort();
...
#[cfg(feature = "nova")]
fn navigate_graveyard(key: event::KeyEvent, vm: &mut ChimeraVM, app_state: &mut AppState) {
    match key.code {
        KeyCode::Down => {
...
#[cfg(feature = "nova")]
fn navigate_alchemy(key: event::KeyEvent, vm: &mut ChimeraVM, app_state: &mut AppState) {
    match key.code {
        KeyCode::Down => {
...
#[cfg(feature = "nova")]
fn navigate_laboratory(key: event::KeyEvent, vm: &mut ChimeraVM, app_state: &mut AppState) {
    match key.code {
        KeyCode::Down => {
...
#[cfg(feature = "nova")]
fn navigate_grimoire(key: event::KeyEvent, vm: &mut ChimeraVM, app_state: &mut AppState) {
    match key.code {
        KeyCode::Down => {
...
#[cfg(feature = "nova")]
fn navigate_dream(key: event::KeyEvent, vm: &mut ChimeraVM, app_state: &mut AppState) {
    match key.code {
        KeyCode::Down => {
...
#[cfg(feature = "nova")]
fn navigate_bestiary(key: event::KeyEvent, vm: &mut ChimeraVM, app_state: &mut AppState) {
    match key.code {
        KeyCode::Down => {
            if !vm.organelles.is_empty() {
...
fn handle_navigation(key: event::KeyEvent, vm: &mut ChimeraVM, app_state: &mut AppState) {
    match app_state.view_mode {
        ViewMode::Genome => navigate_genome(key, vm, app_state),
        ViewMode::Grid => navigate_grid(key, vm, app_state),
        #[cfg(feature = "biophysics")]
        ViewMode::Cortex => navigate_cortex(key, vm, app_state),
        #[cfg(feature = "nova")]
        ViewMode::Graveyard => navigate_graveyard(key, vm, app_state),
        #[cfg(feature = "nova")]
        ViewMode::Alchemy => navigate_alchemy(key, vm, app_state),
        #[cfg(feature = "nova")]
        ViewMode::Laboratory => navigate_laboratory(key, vm, app_state),
        #[cfg(feature = "nova")]
        ViewMode::Grimoire => navigate_grimoire(key, vm, app_state),
        #[cfg(feature = "nova")]
        ViewMode::Dream => navigate_dream(key, vm, app_state),
        #[cfg(feature = "nova")]
        ViewMode::Bestiary => navigate_bestiary(key, vm, app_state),
        _ => {}
    }
}
