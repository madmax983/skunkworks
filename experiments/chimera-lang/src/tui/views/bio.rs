    ...
    if let Some(coord) = app_state.selected_neuron_coords {
        if let Some(neuron) = vm.neurons.get(&coord) {
            let details = vec![
                ...
                Line::from(vec![
                    Span::raw("Last Spike: "),
                    Span::raw(neuron.last_spike.to_string()),
                ]),
            ];
...
pub(crate) fn render_evolution(f: &mut Frame, _vm: &mut ChimeraVM, app_state: &AppState) {
    ...
    if let Some(engine) = &app_state.evolution_state.engine {
        ...
        let stats = vec![
            ...
            Line::from(vec![
                Span::raw("Generation: "),
                Span::raw(engine.generation.to_string()),
            ]),
            Line::from(vec![
                Span::raw("Best Fitness: "),
                Span::raw(engine.best_fitness.to_string()),
            ]),
            Line::from(format!(
                "Challenge: {}",
            ...
            Line::from(vec![
                Span::raw("Population: "),
                Span::raw(engine.population.len().to_string()),
            ]),
            Line::from(vec![
                Span::raw("Auto-Run: "),
                Span::raw(app_state.evolution_state.auto_run.to_string()),
            ]),
            Line::from(" "),
            Line::from("Controls:"),
        ...
        #[cfg(feature = "nova")]
        f.render_widget(
            tui_shared::Button::new("Initialize Evolution Engine (E)").style(
...
pub(crate) fn render_ecology(f: &mut Frame, vm: &mut ChimeraVM, app_state: &AppState) {
    ...
    let mut info = vec![
        ...
        Line::from(vec![
            Span::raw("Organisms: "),
            Span::raw(vm.organelles.len().to_string()),
        ]),
        Line::from(vec![
            Span::raw("Shared Energy: "),
            Span::raw(vm.energy.to_string()),
        ]),
        Line::from(" "),
    ];
    ...
    for org in &vm.organelles {
        if org.context_loc == (cy, cx) {
            info.push(Line::from(vec![
                Span::raw("Name: "),
                Span::raw(org.name.to_string()),
            ]));
            info.push(Line::from(vec![
                Span::raw("ID: "),
                Span::raw(org.id.to_string()),
            ]));
            info.push(Line::from(format!("Type: {:?}", org.kind)));
            info.push(Line::from(vec![
                Span::raw("Energy: "),
                Span::raw(org.energy.to_string()),
            ]));
...
#[cfg(feature = "nova")]
/// Renders the Metazoa view for observing complex multicellular organisms.
///
/// Shows higher-order biology metrics, specifically how multiple independent
/// strands or cells act as a cohesive entity in the simulation grid.
///
/// # Examples
///
/// ```ignore
...
/// Renders the Mutagen view for analyzing radiation or environmental hazards.
///
/// Displays the current levels of mutagens in the environment and their
/// expected effects on active DNA strands and reproduction rates.
///
/// # Examples
///
/// ```ignore
...
pub(crate) fn render_mutagen(f: &mut Frame, vm: &mut ChimeraVM, app_state: &AppState) {
    ...
    if let Some(strand) = vm.dna.helix.strands.get(app_state.selected_strand) {
        info.push(Line::from(vec![
            Span::raw("Strand: "),
            Span::raw(app_state.selected_strand.to_string()),
        ]));
        if let Some(gene) = strand.genes.get(app_state.selected_gene) {
            info.push(Line::from(" "));
            info.push(Line::from(vec![
                Span::raw("Selected Gene: "),
                Span::raw(gene.op.to_string()),
            ]));
...
#[cfg(feature = "nova")]
/// Renders the Biolum view for tracking energy and light emission.
///
/// Visualizes the local light/energy fields generated by organelles
/// like chloroplasts or explicit luminescence commands.
///
/// # Examples
///
/// ```ignore
...
#[cfg(feature = "nova")]
/// Renders the Cambrian view highlighting explosion-phase evolution.
///
/// Provides a historical graph or metrics concerning the rapid generation
/// of new species and DNA sequences over the last few epochs.
///
/// # Examples
///
/// ```ignore
...
pub(crate) fn render_cambrian(f: &mut Frame, vm: &mut ChimeraVM, app_state: &AppState) {
    ...
    let info = vec![
        ...
        Line::from(vec![
            Span::raw("Ch A (Red):   "),
            Span::raw(h[0].to_string()),
        ]),
        Line::from(vec![
            Span::raw("Ch B (Green): "),
            Span::raw(h[1].to_string()),
        ]),
        Line::from(vec![
            Span::raw("Ch C (Blue):  "),
            Span::raw(h[2].to_string()),
        ]),
        Line::from(" "),
        Line::from("Opcodes:"),
...
pub(crate) fn render_biomesh(f: &mut Frame, vm: &mut ChimeraVM, app_state: &AppState) {
    ...
    if let Some(node) = vm.biomesh.nodes.get(&(cy, cx)) {
        ...
        info.push(Line::from(vec![
            ...
            Span::raw(node.buffer.len().to_string()),
        ]));
...
#[cfg(feature = "nova")]
/// Renders the Lifecycle view tracking the aging and phases of organisms.
///
/// This view gives an overview of cellular phases (G1, S, G2, M) and
/// tracks telomere degradation leading to apoptosis.
///
/// # Examples
///
/// ```ignore
