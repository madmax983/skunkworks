...
pub(crate) fn render_prologue_esolang(f: &mut Frame, vm: &mut ChimeraVM, app_state: &AppState) {
    ...
    let mut notes = vec![
        ...
        Line::from(""),
        Line::from(vec![
            Span::raw("🧬 Current Energy: "),
            Span::raw(vm.energy.to_string()),
            Span::raw(" J"),
        ]),
        Line::from(vec![
            Span::raw("🍄 Entropy Level: "),
            Span::raw(vm.glitch_level.to_string()),
        ]),
        Line::from(""),
...
pub(crate) fn render_akashic(f: &mut Frame, vm: &mut ChimeraVM, app_state: &AppState) {
    ...
    let info = vec![
        Line::from(vec![
            Span::raw("Karma: "),
            Span::raw(vm.akashic.karma.to_string()),
        ]),
        Line::from(" "),
...
pub(crate) fn render_prologue(f: &mut Frame, vm: &mut ChimeraVM, app_state: &AppState) {
    ...
    info.push(Line::from(vec![
        Span::raw("Active Signals: "),
        Span::raw(signal_count.to_string()),
    ]));
...
pub(crate) fn render_forge(f: &mut Frame, vm: &mut ChimeraVM, app_state: &AppState) {
    ...
    let test_text = vec![
        Line::from(vec![
            Span::raw("Input: "),
            Span::raw(app_state.forge_test_input.to_string()),
        ]),
        Line::from("---"),
        Line::from(vec![
            Span::raw("Output: "),
            Span::raw(app_state.forge_test_output.to_string()),
        ]),
    ];
...
pub(crate) fn render_dream(f: &mut Frame, vm: &mut ChimeraVM, app_state: &AppState) {
    ...
    if !vm.dream_traces.is_empty() && app_state.selected_dream_trace < vm.dream_traces.len() {
        ...
        let mut info_text = vec![
            Line::from(vec![
                Span::raw("Mutation: "),
                Span::raw(trace.mutation_desc.to_string()),
            ]),
            Line::from(format!(
                "Energy: {} -> {} (Cost: {})",
            ...
            Line::from(vec![
                Span::raw("Accepted: "),
                Span::raw(trace.accepted.to_string()),
            ]),
        ];
...
pub(crate) fn render_kaleidoscope(f: &mut Frame, vm: &mut ChimeraVM, app_state: &AppState) {
    ...
    if let Some(state) = &vm.piet_state {
        ...
        info_lines.push(Line::from(vec![
            Span::raw("Steps: "),
            Span::raw(state.steps.to_string()),
        ]));
...
pub(crate) fn render_bestiary(f: &mut Frame, vm: &mut ChimeraVM, app_state: &AppState) {
    ...
    if !vm.organelles.is_empty() && app_state.selected_organelle_index < vm.organelles.len() {
        ...
        let stats = vec![
            ...
            Line::from(vec![
                Span::raw("Stack Depth: "),
                Span::raw(org.stack.len().to_string()),
            ]),
