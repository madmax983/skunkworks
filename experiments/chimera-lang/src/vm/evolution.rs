...
/// Defines the specific goal or environment for a running evolutionary simulation.
pub enum Challenge {
    /// Demands the organism's output sequence successfully evaluate to the specified numerical target.
    Target(i64),
    /// Subjects the organism to a fitness test where its output must equal exactly twice its numerical input.
    Doubler,
    /// Evaluates the organism's ability to consume two inputs and yield their sum on the stack.
    Adder,
    /// Tests whether the organism can correctly calculate and produce the Fibonacci sequence up to a given iteration.
    Fibonacci,
    /// Defines a user-supplied fitness evaluation criteria structure to override standard environments.
...
#[derive(Clone)]
/// Drives the evolutionary algorithms, managing the population and fitness evaluation.
...
impl EvolutionEngine {
    ...
    pub fn new(seed: Strand, population_size: usize, challenge: Challenge) -> Self {
        ...
        for item in population.iter_mut().take(safe_pop_size).skip(1) {
            Self::mutate_strand(item, &mut rng);
        }
