...
pub fn exec_nova_op(vm: &mut ChimeraVM, op: OpCode, args: &[Nucleotide]) -> Option<(usize, usize)> {
    match op {
        ...
        OpCode::Interfere => super::nova_hologram::exec_interfere(vm, op, args),
        OpCode::Project => super::nova_hologram::exec_project(vm, op, args),
        OpCode::Refract => super::nova_hologram::exec_refract(vm, op, args),
...
pub(crate) fn exec_glitch_art(vm: &mut ChimeraVM) -> Option<(usize, usize)> {
    ...
    vm.output
        .push(format!("🎨 Glitch Art generated: {}", value));
    None
}
...
pub fn exec_chromatic_code(vm: &mut ChimeraVM) {
    ...
    vm.output
        .push("CHROMATIC_CODE: Carrier Signal Detected 🌈🔐".to_string());
