                        ...
                        if len > 0 {
                            ...
                            for k in 0..max_len {
                                ...
                                if (0..16).contains(&y) && curr_x < 16 {
                                    sequence.push(vm.grid[y as usize][curr_x].clone());
                                } else {
                                    valid = false;
                                    vm.output
                        ...
                        Value::Int(amount) => {
                            if amount > 0 {
                                ...
                                if idx < vm.telomeres.len() {
                                    vm.telomeres[idx] =
                                        vm.telomeres[idx].saturating_add(amount);
                        ...
                        if s_idx < vm.dna.helix.strands.len() {
                            ...
                            for val in cells_to_write {
                                if (0..16).contains(&curr_x) && (0..16).contains(&curr_y) {
                                    vm.grid[curr_y as usize][curr_x as usize] = val;
                                    success_count += 1;
                                    curr_x += dx;
                                    curr_y += dy;
                                } else {
                ...
                if vm.stack.len() >= 2 {
                    ...
                    if let (Value::Int(r), Value::Int(intensity)) = (radius_val, intensity_val) {
                        if r > 0 && intensity > 0 {
                            ...
                            for (tx, ty) in coords {
                                vm.light_grid[ty][tx] =
                                    vm.light_grid[ty][tx].saturating_add(intensity);
                            }
