...
pub fn exec_secrete(vm: &mut ChimeraVM) -> Option<(usize, usize)> {
    if vm.stack.len() >= 2 {
        let Some(amount_val) = vm.stack.pop() else {
            return None;
        };
        let Some(channel_val) = vm.stack.pop() else {
            return None;
        };
...
pub fn exec_match(vm: &mut ChimeraVM) -> Option<(usize, usize)> {
    if vm.stack.len() >= 2 {
        let Some(target_val) = vm.stack.pop() else {
            return None;
        };
        let Some(pattern_val) = vm.stack.pop() else {
            return None;
        };
