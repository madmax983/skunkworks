...
pub fn exec_morphogen(vm: &mut ChimeraVM, _op: OpCode, _args: &[Nucleotide]) -> Option<(usize, usize)> {
    ...
    if vm.stack.len() >= 2 {
        ...
        if let (Value::Int(c), Value::Int(a)) = (chan_val, amt_val) {
            ...
            vm.output.push(format!("MORPHOGEN: Emit Ch{} Amt{} at {},{}", channel, amount, cx, cy));
        } else {
            vm.output.push("Error: Type mismatch for Morphogen".to_string());
        }
    } else {
        vm.output.push("Error: Stack underflow for Morphogen".to_string());
    }
    None
}

pub fn exec_hox_switch(vm: &mut ChimeraVM, _op: OpCode, _args: &[Nucleotide]) -> Option<(usize, usize)> {
    ...
    if vm.stack.len() >= 3 {
        ...
        if let (Value::Int(c), Value::Int(t), Value::Int(s_idx)) = (chan_val, thresh_val, strand_val) {
            let channel = c.clamp(0, 2) as usize;
            let (cy, cx) = vm.context_loc;
            ...
            if level > t {
                ...
                if target < vm.dna.helix.strands.len() {
                    ...
                    vm.output.push(format!("HOX: Activated Ch{} ({} > {}) -> Jump {}", channel, level, t, target));
                    return Some((target, 0));
                } else {
                    vm.output.push("Error: Invalid strand index for HoxSwitch".to_string());
                }
            }
        } else {
            vm.output.push("Error: Type mismatch for HoxSwitch".to_string());
        }
    } else {
        vm.output.push("Error: Stack underflow for HoxSwitch".to_string());
    }
    None
}
