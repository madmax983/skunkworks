    ...
    if let Some(val) = vm.stack.pop() {
        if let Value::Int(r) = val {
            if r >= 0 {
                ...
                vm.output.push(format!("POCKET: Captured radius {} at {},{}", radius, cx, cy));
            } else {
                vm.output.push("POCKET: Radius must be non-negative".to_string());
            }
        } else {
            vm.output.push("Error: Type mismatch for pocket".to_string());
        }
    } else {
        vm.output.push("Error: Stack underflow for pocket".to_string());
    }
    None
}
                ...
                if let (Value::Str(marker), Value::Int(r)) = (&items[0], &items[1]) {
                    if marker == "POCKET" {
                        ...
                        if items.len() == 2 + expected_size {
                            ...
                            vm.output.push(format!("UNPOCKET: Released radius {} at {},{}", radius, cx, cy));
                        } else {
                            vm.output.push("UNPOCKET: Corrupted pocket data size".to_string());
                        }
                    } else {
                        vm.output.push("UNPOCKET: Invalid junction marker".to_string());
                    }
                } else {
                    vm.output.push("UNPOCKET: Invalid pocket header format".to_string());
                }
            } else {
                vm.output.push("UNPOCKET: Invalid pocket structure".to_string());
            }
        } else {
            vm.output.push("Error: Type mismatch for unpocket (expected Junction)".to_string());
        }
    } else {
        vm.output.push("Error: Stack underflow for unpocket".to_string());
    }
    None
}
