...
fn test_glossolalia_generate() {
    ...
    let genes = vec![
        ...
        Gene { op: OpCode::ParserMatch, args: vec![] },
        Gene { op: OpCode::ParserSeq, args: vec![] },
        Gene { op: OpCode::Generate, args: vec![] },
    ];
...
fn test_glossolalia_scribe() {
    ...
    let genes = vec![
        Gene { op: OpCode::Push, args: vec![Nucleotide::String("TheTablet".to_string())] },
...
fn test_oracle_generate() {
    ...
    let grammar = Value::Junction(chimera_lang::ast::JunctionType::Any, vec![
        Value::Str("Match".to_string()),
        Value::Str("Oracle".to_string())
    ]);

    let goal = Value::Junction(chimera_lang::ast::JunctionType::Any, vec![
    ...
    oracle::solve(&[goal], HashMap::new(), &vm.knowledge_base, &vm, &mut solutions, 0);
