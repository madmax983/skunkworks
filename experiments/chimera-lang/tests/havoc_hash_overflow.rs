...
fn test_hash_overflow_exploit() {
    ...
    assert!(!status.success(), "The system was expected to crash, but it survived! Havoc failed.");
    ...
    #[cfg(unix)]
    {
        ...
        assert!(sig == Some(6) || sig == Some(11), "Expected SIGABRT (6) or SIGSEGV (11) from stack overflow, got {:?}", sig);
    }
}
