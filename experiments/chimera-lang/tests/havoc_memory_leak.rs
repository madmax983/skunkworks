...
mod tests {
    ...
    fn test_cladistics_memory_leak() {
        ...
        // Give energy
        genes.push(Gene { op: OpCode::Push, args: vec![Nucleotide::Number(10000)] });
        ...
        for _ in 0..5 {
            ...
            genes.push(Gene { op: OpCode::Mitosis, args: vec![] });
        }
        genes.push(Gene { op: OpCode::Singularity, args: vec![] });
        ...
        assert!(final_nodes < 100, "Memory leak detected: Cladistics nodes grew to {}", final_nodes);
    }
}
