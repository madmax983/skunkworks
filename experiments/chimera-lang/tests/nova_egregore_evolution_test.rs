...
fn test_egregore_evolution() {
    ...
    // Verify
    assert!(vm.egregore.alignment > 0, "Alignment should increase (Order)");
...
fn test_egregore_sacrifice() {
    ...
    let genes = vec![
        Gene { op: OpCode::Sacrifice, args: vec![] },
    ];
    ...
    assert!(vm.egregore.alignment < 0, "Alignment should decrease (Chaos)");
    assert!(vm.halted, "VM should halt after sacrifice (if last strand died)");
    assert!(vm.dna.helix.strands[0].genes.is_empty(), "Strand should be empty");
}
