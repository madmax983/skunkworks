...
mod tests {
    ...
    fn test_astral_agent_spawn_and_gravity() {
        ...
        println!("Tick 1: Pos({:.2}, {:.2}) Vel({:.4}, {:.4})", state.px, state.py, state.vx, state.vy);
        assert!(state.vx > 0.0, "Agent should accelerate towards gravity well (East)");
        ...
        let agent = &vm.prologue_state.agents[agent_idx.unwrap()];
        let state = AstralState::from_value(&agent.state);
        println!("Tick 6: Pos({:.2}, {:.2}) Vel({:.4}, {:.4})", state.px, state.py, state.vx, state.vy);
