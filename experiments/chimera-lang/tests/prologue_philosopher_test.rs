...
fn test_philosopher_agent_movement() {
    ...
    let head = Value::Junction(JunctionType::Any, vec![
    ...
    // Body 1: goal("explore")
    let body1 = Value::Junction(JunctionType::Any, vec![
        Value::Str("goal".to_string()),
        Value::Str("explore".to_string())
    ]);

    // Body 2: neighbor("east", 0)
    let body2 = Value::Junction(JunctionType::Any, vec![
        Value::Str("neighbor".to_string()),
        Value::Str("east".to_string()),
        Value::Int(0)
    ]);

    let rule = Value::Junction(JunctionType::Any, vec![
    ...
    assert_eq!(vm.grid[5][6], Value::Str("Φ".to_string()), "Philosopher should have moved East");
