    ...
    fn test_synaptic_bridge() {
        let strand0 = Strand {
            genes: vec![
                ...
                Gene { op: OpCode::Push, args: vec![Nucleotide::Number(1)] }, // Weight
                Gene { op: OpCode::Push, args: vec![Nucleotide::Number(5)] }, // y
                Gene { op: OpCode::Push, args: vec![Nucleotide::Number(5)] }, // x
                Gene { op: OpCode::NeuroCoupling, args: vec![] },

                // 3. Synapse to Strand 1
                Gene { op: OpCode::Push, args: vec![Nucleotide::Number(1)] }, // strand_idx
                Gene { op: OpCode::Push, args: vec![Nucleotide::Number(5)] }, // y
                Gene { op: OpCode::Push, args: vec![Nucleotide::Number(5)] }, // x
                Gene { op: OpCode::NeuroSynapse, args: vec![] },
        ...
        assert!(vm.biophysics_couplings.contains_key(&(5,5)), "Coupling not set");
        assert!(vm.neuron_to_cortex_map.contains_key(&(5,5)), "Synapse not created");
        ...
        assert!(n.v != -65.0, "Coupling failed: Neuron potential did not change.");
        ...
        assert!(activation > 0, "Synapse failed: Cortex strand not activated after spike.");
    }
}
