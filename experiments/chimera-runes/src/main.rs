...
impl Genome {
    ...
    fn crossover(a: &Self, b: &Self) -> Self {
        let mut rng = rand::thread_rng();
        if a.dna.is_empty() || b.dna.is_empty() {
            return Self::random();
        }
        ...
        if new_dna.len() > 256 {
            new_dna.truncate(256);
        }
        if new_dna.len() < 16 {
            while new_dna.len() < 16 {
                new_dna.push(rng.gen());
            }
        }
...
fn ui(f: &mut Frame, p1: &Genome, p2: &Genome, child: &Genome, output: &str) {
    ...
    .block(
        Block::default()
            .borders(Borders::ALL)
            .title("Chimera Runes Lab"),
    );
    f.render_widget(help, bottom_chunks[0]);

    let log =
        Paragraph::new(output).block(Block::default().borders(Borders::ALL).title("VM Output"));
