use crate::gait::{calculate_gait, GaitParameters};
use crate::terrain::{Terrain, NODE_GAP, NODE_WIDTH, START_X};
use crate::brain::ChimeraBrain;
...
pub fn spawn_walker(mut commands: Commands) {
    commands.spawn((Walker::default(), ChimeraBrain::default()));
}

pub fn walker_system(mut query: Query<(&mut Walker, &mut ChimeraBrain)>, terrain: Res<Terrain>, time: Res<Time>) {
    let dt = time.delta_seconds();

    for (mut walker, mut brain) in &mut query {
        ...
        // CHIMERA INTEGRATION
        // 1. Feed Input (Terrain Type)
        // 0 = Directory, 1 = File
        let terrain_type = if !terrain.nodes.is_empty() {
            match terrain.nodes[current_node_idx].file_type {
                crate::terrain::FileType::Directory => 0,
                crate::terrain::FileType::File => 1,
            }
        } else {
            1
        };

        // Write Input to Grid[0][0]
        brain.vm.grid[0][0] = Value::Int(terrain_type);

        // 2. Step VM
        // Run a few steps per frame to allow computation
        for _ in 0..10 {
            brain.vm.step();
        }

        // 3. Read Output
        // Speed at (0,1), Bounce at (0,2), Height at (0,3)
        let speed_val = match &brain.vm.grid[0][1] { Value::Int(v) => *v as f32, _ => 50.0 };
        let bounce_val = match &brain.vm.grid[0][2] { Value::Int(v) => *v as f32, _ => 5.0 };
        let height_val = match &brain.vm.grid[0][3] { Value::Int(v) => *v as f32, _ => 20.0 };

        let gait = GaitParameters {
            speed: speed_val.clamp(10.0, 300.0),
            bounce: bounce_val.clamp(0.0, 100.0),
            step_height: height_val.clamp(5.0, 150.0),
        };
        ...
        // Stride is roughly based on speed * duration
