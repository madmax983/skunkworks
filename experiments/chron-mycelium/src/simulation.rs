...
impl World {
    ...
    pub fn with_food_and_agents(
        width: usize,
        height: usize,
        num_agents: usize,
        food_sources: &[(f64, f64)],
    ) -> (Self, Vec<Agent>) {
        let mut rng = rand::thread_rng();
        ...
        for _ in 0..num_agents {
            let home_idx = rng.gen_range(0..num_cities);
            let cx = cities[home_idx].0;
            let cy = cities[home_idx].1;

            let mut target = rng.gen_range(0..num_cities);
            if num_cities > 1 {
                while target == home_idx {
