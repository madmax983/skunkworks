...
fn draw_gear(gear: &Gear, x: f32, y: f32, color: Color) {
    ...
    for i in 0..teeth {
        ...
        let p1 = vec2(x + inner_r * (theta - 0.1).cos(), y + inner_r * (theta - 0.1).sin());
        let p2 = vec2(x + r * (theta - 0.05).cos(), y + r * (theta - 0.05).sin());
        let p3 = vec2(x + r * (theta + 0.05).cos(), y + r * (theta + 0.05).sin());
        let p4 = vec2(x + inner_r * (theta + 0.1).cos(), y + inner_r * (theta + 0.1).sin());
...
async fn main() {
    ...
    loop {
        ...
        draw_text(&format!("Current Input: {}", machine.current_input_char), 20.0, 60.0, 20.0, GRAY);
        ...
        draw_line(input_pos.x, input_pos.y, output_pos.x, output_pos.y, 5.0, DARKGRAY);
        draw_line(key_pos.x, key_pos.y, output_pos.x, output_pos.y, 5.0, DARKGRAY);

        // Draw Gears
        draw_gear(&machine.train.gears[machine.input_handle], input_pos.x, input_pos.y, GREEN);
        draw_text("Input", input_pos.x - 20.0, input_pos.y + 70.0, 20.0, GREEN);

        draw_gear(&machine.train.gears[machine.drive_shaft], drive_pos.x, drive_pos.y, BLUE);
        draw_text("Time", drive_pos.x - 20.0, drive_pos.y + 70.0, 20.0, BLUE);

        draw_gear(&machine.train.gears[machine.key_rotor], key_pos.x, key_pos.y, YELLOW);
        draw_text("Key", key_pos.x - 20.0, key_pos.y + 70.0, 20.0, YELLOW);

        draw_gear(&machine.train.gears[machine.diff_output], output_pos.x, output_pos.y, RED);
        draw_text("Output", output_pos.x - 20.0, output_pos.y + 70.0, 20.0, RED);
        ...
        let normalized = (angle % (2.0 * std::f32::consts::PI) + 2.0 * std::f32::consts::PI) % (2.0 * std::f32::consts::PI);
        let step = 2.0 * std::f32::consts::PI / 26.0;
        let idx = (normalized / step).round() as i32 % 26;
        let out_char = (b'A' + idx as u8) as char;

        draw_text(&format!("-> {}", out_char), output_pos.x - 20.0, output_pos.y - 70.0, 40.0, RED);

        draw_text("Type A-Z to encrypt", 20.0, screen_height() - 20.0, 20.0, GRAY);
