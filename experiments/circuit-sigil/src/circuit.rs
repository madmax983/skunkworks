...
mod tests {
    ...
    #[test]
    fn test_no_pad_collisions() {
        let generator = CircuitGenerator::new(512, 512);
        let (_, pads) = generator.generate("test-hash-collision");

        let min_dist_sq = (6 * 2 + 8) * (6 * 2 + 8);
        ...
        for i in 0..pads.len() {
            for j in (i+1)..pads.len() {
                ...
                assert!(dist_sq >= min_dist_sq, "Pads overlap! dist_sq: {}, min: {}", dist_sq, min_dist_sq);
            }
        }
    }
}
