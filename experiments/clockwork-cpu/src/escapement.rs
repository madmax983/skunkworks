...
fn spawn_escapement(mut commands: Commands) {
    ...
    let wheel_entity = commands.spawn((
        ShapeBundle {
            path: wheel_path,
            spatial: SpatialBundle::from_transform(Transform::from_translation(wheel_pos.extend(0.0))),
            ..default()
        },
        Fill::color(Color::GOLD),
        Stroke::new(Color::BLACK, 2.0),
        Gear { teeth: escape_wheel_teeth, radius: escape_wheel_radius },
        EscapeWheel,
        RigidBody::Dynamic,
    ...
    commands.entity(wheel_entity).insert(Damping { angular_damping: 1.0, linear_damping: 0.0 });
    ...
    for i in 0..escape_wheel_teeth {
        ...
        let tip_pos = Vec2::new(angle.cos() * escape_wheel_radius, angle.sin() * escape_wheel_radius);

        let tooth = commands.spawn((
    ...
    commands.entity(wheel_entity).insert(ImpulseJoint::new(ground, joint));
    ...
    let anchor_entity = commands.spawn((
        ...
        RigidBody::Dynamic,
        Verge,
        Damping { angular_damping: 0.5, linear_damping: 0.0 }, // Friction
    )).id();
    ...
    let anchor_visual = commands.spawn((
        ShapeBundle {
            path: anchor_shape,
            ..default()
        },
    ...
    let left_pallet = commands.spawn((
        TransformBundle::from_transform(Transform::from_translation(left_pallet_pos.extend(0.0))),
        Collider::cuboid(5.0, 20.0),
    )).id();

    let right_pallet_pos = Vec2::new(40.0, -30.0);
    let right_pallet = commands.spawn((
        TransformBundle::from_transform(Transform::from_translation(right_pallet_pos.extend(0.0))),
        Collider::cuboid(5.0, 20.0),
    )).id();
    ...
    commands.entity(anchor_entity).insert(ImpulseJoint::new(ground, anchor_joint));
