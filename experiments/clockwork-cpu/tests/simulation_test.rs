...
fn test_clockwork_movement() {
    ...
    // Initial check
    let initial_wheel_rot = world
        .rigid_body_set
        .get(clockwork.wheel_handle)
        .unwrap()
        .rotation()
        .angle();
    let initial_anchor_rot = world
        .rigid_body_set
        .get(clockwork.anchor_handle)
        .unwrap()
        .rotation()
        .angle();
    ...
    let final_wheel_rot = world
        .rigid_body_set
        .get(clockwork.wheel_handle)
        .unwrap()
        .rotation()
        .angle();
    let final_anchor_rot = world
        .rigid_body_set
        .get(clockwork.anchor_handle)
        .unwrap()
        .rotation()
    ...
    assert!(
        (final_wheel_rot - initial_wheel_rot).abs() > 0.01,
        "Wheel should have moved"
    );
    ...
    assert!(
        (final_anchor_rot - initial_anchor_rot).abs() > 0.001,
        "Anchor should have moved"
    );
}
