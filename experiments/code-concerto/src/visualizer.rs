...
impl Visualizer {
    pub fn new(concerto: CodeConcerto) -> Self {
        ...
        for (i, section) in concerto.sections.iter().enumerate() {
            ...
            if dur < 0.1 { dur = 1.0; } // Minimum duration fallback
    ...
    pub fn update(&mut self, dt: f64) {
        ...
        if active_building_idx.is_none() && !self.buildings.is_empty() {
             ...
             if self.current_time >= last.start_time + last.duration {
                 ...
                 target_pos = last.position + vec3(last.size.x/2.0 + time_past as f32 * 5.0, last.size.y/2.0 + 2.0, 0.0);
             }
        }
    ...
    pub fn draw(&self) {
        ...
        for b in &self.buildings {
            ...
            draw_cube_wires(b.position, b.size, BLACK);
             ...
             // Shadow/Reflection
             draw_cube(vec3(b.position.x, -0.5, b.position.z), vec3(b.size.x, 0.1, b.size.z), None, Color::new(b.color.r, b.color.g, b.color.b, 0.2));
        ...
        // Beam down
        draw_line_3d(self.playhead_pos, vec3(self.playhead_pos.x, 0.0, self.playhead_pos.z), WHITE);

        set_default_camera();

        // HUD
        draw_text(&format!("Time: {:.1}s", self.current_time), 20.0, 30.0, 30.0, WHITE);

        // Show current section name if any
        for b in &self.buildings {
             if self.current_time >= b.start_time && self.current_time < (b.start_time + b.duration) {
                 if let Some(section) = self.concerto.sections.get(b.section_index) {
                     draw_text(&format!("Now Playing: {}", section.name), 20.0, 60.0, 30.0, YELLOW);
                     draw_text(&format!("Type: {:?}", section.kind), 20.0, 90.0, 20.0, LIGHTGRAY);
                 }
                 break;
             }
        }
