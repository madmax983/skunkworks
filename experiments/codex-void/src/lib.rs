...
mod tests {
    ...
    fn test_encode_decode_cycle() {
        ...
        let decoded_text = String::from_utf8(decoded_bytes).expect("Decoded bytes should be valid UTF-8");

        assert_eq!(text, decoded_text, "Decoded text should match original text");
    }
}
