...
use crate::model::{MarketState, ThreadAgent, Core, AgentState};
...
pub fn draw_ui(
    ...
    let header_text = vec![
        Line::from(vec![
            Span::styled(" CPU CASINO ", Style::default().fg(Color::Magenta).add_modifier(Modifier::BOLD)),
            Span::raw(format!("| Tick: {} | Price: {:.2} | Active: {} | Killed: {} | Finished: {}",
                state.current_tick, state.current_price, state.active_thread_count, state.killed_count, state.finished_count)),
        ]),
        Line::from(vec![
    ...
    // Market Graph
    let price_data: Vec<u64> = state.price_history.iter().map(|&x| (x * 100.0) as u64).collect();
    let sparkline = Sparkline::default()
        .block(Block::default().title(" Market Price (cents) ").borders(Borders::ALL))
        .data(&price_data)
    ...
    let rows: Vec<Row> = agents.iter()
        ...
        .map(|agent| {
            ...
            let color = match agent.state {
                ...
                AgentState::Active => if urgency > 0.9 { Color::Red } else { Color::White },
            };
    ...
    .header(header)
    .block(Block::default().title(" Active Threads ").borders(Borders::ALL));

    f.render_widget(table, body_chunks[0]);

    // Cores
    let core_list: Vec<ListItem> = cores.iter().map(|core| {
        let content = if let Some(agent_id) = core.current_agent_id {
            format!(" CORE {:02}: RUNNING Agent {:04} (Util: {:.1}%)", core.id, agent_id, core.utilization * 100.0)
        } else {
            format!(" CORE {:02}: IDLE (Util: {:.1}%)", core.id, core.utilization * 100.0)
        };
    ...
    let cores_widget = List::new(core_list)
