...
fn window_conf() -> Conf {
    Conf {
        window_title: "Crumpled Memory 🦢🧠".to_string(),
        window_width: 1280,
        window_height: 720,
        high_dpi: true,
        sample_count: 4,
...
async fn main() {
    ...
    // We want the mesh resolution to be reasonable.
    // The image texture will be mapped onto this mesh.
    let cols = 30;
    let rows = 30;
    ...
    loop {
        ...
        // Apply stress if folded significantly
        // The stress is applied along the grid lines.
        let stress_factor = (1.0 - extension).max(0.0);
        let fold_delta = (extension - old_extension).abs();

        // Only stress if moving/folding or if deeply folded
        if fold_delta > 0.0001 || stress_factor > 0.8 {
            let amount = stress_factor * 0.05 + fold_delta * 2.0; // Movement causes more damage

            // Apply along grid lines
            // Map grid (cols, rows) to image (IMG_WIDTH, IMG_HEIGHT)
            ...
            for j in 0..=rows {
                ...
                memory.stress_line(0, y, IMG_WIDTH, y, amount);
            }

            for i in 0..=cols {
                let x = (i as f32 * dx) as u32;
                memory.stress_line(x, 0, x, IMG_HEIGHT, amount);
            }
        }
