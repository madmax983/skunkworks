...
pub fn scan_dependencies(root: &Path) -> Result<Graph> {
    ...
    // Re-iterate over collected nodes to avoid borrowing issues while mutating edges
    // Actually we can just build a list of edges.

    // To lookup target IDs by path, we need the `path_to_id` map.
    // To lookup target IDs by name, we need `name_to_ids` map.

    // We need to read file content again or store it. Storing it might be heavy.
    // Let's read again.
    for node in &nodes {
