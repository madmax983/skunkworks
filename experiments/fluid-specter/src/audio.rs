...
mod tests {
    ...
    fn test_ghost_mode() {
        ...
        // In this test env without audio device, it should fall back to Ghost Mode or just work.
        // We verify that get_spectrum returns correct size and non-zero data.
        let spectrum = audio.get_spectrum();
        assert_eq!(spectrum.len(), FFT_SIZE / 2);

        // Ghost mode should produce some non-zero values
        // If real audio is silent, it might be near zero, but ghost mode definitely has signals.
        // We can't easily force ghost mode without mocking cpal or using cfg,
        // but since we are likely in headless, it uses ghost mode.
        // Even if it uses real audio, mic noise > 0 usually.
