...
mod tests {
    ...
    #[test]
    fn test_char_to_terrain_p() {
        let terrain = char_to_terrain('p');
        // 'p' should be a high peak and hard (voiceless stop)
        assert!(terrain.height > 0.8, "p should be high, got {}", terrain.height);
        assert!(terrain.hardness > 0.8, "p should be hard, got {}", terrain.hardness);
    }

    #[test]
    fn test_char_to_terrain_a() {
        let terrain = char_to_terrain('a');
        // 'a' should be a low valley (vowel)
        assert!(terrain.height < 0.2, "a should be low, got {}", terrain.height);
    }

    #[test]
    fn test_terrain_to_char_reconstruction() {
        let p_terrain = TerrainPoint { height: 0.9, hardness: 0.9 };
        assert_eq!(terrain_to_char(&p_terrain), 'p');

        let a_terrain = TerrainPoint { height: 0.1, hardness: 0.1 };
        assert_eq!(terrain_to_char(&a_terrain), 'a');
    }
}
