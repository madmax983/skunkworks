...
async fn main() {
    ...
    for _ in 0..10 {
        firewalls.push(Firewall {
            pos: vec2(rng.gen_range(100.0..WIDTH - 100.0), rng.gen_range(100.0..HEIGHT - 100.0)),
    ...
    loop {
        ...
        // Parallel update
        let fw_clone = firewalls.iter().map(|f| (f.pos, f.radius)).collect::<Vec<_>>();
        ...
        packets.par_iter_mut().for_each(|p| {
            ...
            if p.pos.x < -100.0 || p.pos.x > WIDTH + 100.0 || p.pos.y < -100.0 || p.pos.y > HEIGHT + 100.0 {
                p.active = false;
            }
        });
        ...
        for fw in firewalls.iter_mut() {
            let hits = packets.iter().filter(|p| p.active && p.pos.distance(fw.pos) < fw.radius + 15.0).count();
            if hits > 0 {
        ...
        for t in &targets {
            ...
            draw_circle_lines(t.pos.x, t.pos.y, t.radius, 1.0, Color::new(0.0, 1.0, 0.4, 0.8));

            // Only draw text if it's a big target to avoid clutter
            if t.radius > 15.0 {
                let text_size = measure_text(&t.path, None, 16, 1.0);
                draw_text(&t.path, t.pos.x - text_size.width / 2.0, t.pos.y + t.radius + 15.0, 16.0, WHITE);
            }
        }
