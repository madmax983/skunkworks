...
impl GitSource {
    ...
    pub fn read_file_at_revision(&self, path: &str, rev: &str) -> Result<Vec<u8>> {
        ...
        let obj = self.repo.revparse_single(rev).context(format!("Invalid revision '{}'", rev))?;

        // Peel to tree (if commit or tag)
        let tree = obj.peel_to_tree().context(format!("'{}' does not point to a tree-ish object", rev))?;
