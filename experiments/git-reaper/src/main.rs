...
async fn main() {
    ...
    // Load git commits to populate the heap
    if let Ok(commits) = git::get_commit_history() {
        heap.nodes.clear();
        heap.roots.clear();
        heap.next_id = 0;
        ...
        for (i, commit) in commits.iter().enumerate() {
            // Seed random generation with commit hash to make it deterministic
            // per commit, visualizing actual code structure as a unique heap graph.
            let hash_val = commit.hash.chars().fold(0, |acc, c| acc + c as u32);
            let offset_x = (hash_val % 400) as f32 - 200.0;
            let offset_y = ((hash_val / 400) % 400) as f32 - 200.0;

            let x = w / 2.0 + offset_x;
            let y = h / 2.0 + offset_y;

            let id = heap.allocate(vec2(x, y));
            if i == 0 {
                // First commit (newest) is a root
            ...
            // Periodically branch off some dangling branches (to be collected)
            if rng.gen_bool(0.15) {
                let d_id1 = heap.allocate(vec2(x + 50.0, y + 50.0));
                heap.link(id, d_id1);

                let d_id2 = heap.allocate(vec2(x + 100.0, y + 100.0));
                heap.link(d_id1, d_id2);

                // Cut the reference immediately to make it garbage
                if let Some(parent) = heap.nodes.get_mut(&id) {
                    parent.children.retain(|&c| c != d_id1);
                }
                if let Some(child) = heap.nodes.get_mut(&d_id1) {
    ...
    loop {
        ...
        // Draw UI
        draw_text(
            "Git-Reaper: Garbage Collection of Codebase History",
            20.0,
            30.0,
            30.0,
            WHITE,
