use ::rand::Rng;
use gray_scott::GrayScott;
use macroquad::prelude::*;

pub struct GrayString {
    // Physics / Visuals
    pub pos: Vec2,
    pub length: f32,
    pub vibration: f32,
    pub velocity: f32,

    // Properties
    pub frequency: f32, // Hz
    pub base_tension: f32,
    pub tension: f32, // Spring constant k
    pub decay: f32,   // Damping factor (0.0 - 1.0)
}

impl GrayString {
    pub fn new(pos: Vec2, length: f32, base_freq: f32) -> Self {
        let mut rng = ::rand::thread_rng();

        let frequency = base_freq * rng.gen_range(0.9..1.1);
        let base_tension = rng.gen_range(200.0..400.0);
        let decay = rng.gen_range(0.98..0.995);

        Self {
            pos,
            length,
            vibration: 0.0,
            velocity: 0.0,
            frequency,
            base_tension,
            tension: base_tension,
            decay,
        }
    }

    pub fn update_physics(&mut self, dt: f32, gs: &GrayScott, grid_scale: f32) {
        // Sample chemical field at center of string
        let mid_x = self.pos.x;
        let mid_y = self.pos.y + self.length * 0.5;
        let gx = (mid_x / grid_scale) as usize;
        let gy = (mid_y / grid_scale) as usize;

        let mut u_val = 1.0;
        let mut v_val = 0.0;

        if gx < gs.width() && gy < gs.height() {
            let idx = gs.get_index(gx, gy);
            u_val = gs.u()[idx];
            v_val = gs.v()[idx];
        }

        // Chemical U (feed) increases tension.
        // Chemical V (kill) decreases tension.
        self.tension = self.base_tension + (u_val * 100.0) - (v_val * 100.0);
        self.tension = self.tension.max(10.0);

        let damping = 2.0;

        let acceleration = -self.tension * self.vibration - damping * self.velocity;
        self.velocity += acceleration * dt;
        self.vibration += self.velocity * dt;

        // Clamp to avoid explosion
        self.vibration = self.vibration.clamp(-40.0, 40.0);
    }

    pub fn perturb_grid(&self, gs: &mut GrayScott, grid_scale: f32) {
        // The string acts as a catalyst when vibrating.
        // We inject V chemical proportional to vibration.
        if self.vibration.abs() < 0.1 {
            return;
        }

        let steps = 20;
        let step_size = self.length / steps as f32;

        let intensity = (self.vibration.abs() * 0.05).clamp(0.0, 1.0);

        for i in 0..=steps {
            let y_offset = i as f32 * step_size;
            let ratio = y_offset / self.length;
            let shape = (std::f32::consts::PI * ratio).sin();
            let x = self.pos.x + self.vibration * shape;
            let y = self.pos.y + y_offset;

            let gx = (x / grid_scale) as usize;
            let gy = (y / grid_scale) as usize;

            if gx < gs.width() && gy < gs.height() {
                // Add V chemical to the grid
                gs.add_chemical(gx, gy, intensity * shape);
            }
        }
    }

    pub fn pluck(&mut self, strength: f32) {
        self.velocity += strength;
    }

    pub fn draw(&self) {
        let start = self.pos;
        let end = self.pos + vec2(0.0, self.length);

        let segments = 20;
        let step = self.length / segments as f32;

        let mut prev = start;

        let thickness = 2.0;

        let charge = (self.vibration / 20.0).clamp(-1.0, 1.0);
        let color = if charge > 0.0 {
            Color::new(1.0, 1.0 - charge, 1.0 - charge, 1.0)
        } else {
            Color::new(1.0 + charge, 1.0 + charge, 1.0, 1.0)
        };

        for i in 1..=segments {
            let y_offset = i as f32 * step;
            let ratio = y_offset / self.length;
            let shape = (std::f32::consts::PI * ratio).sin();
            let x = self.pos.x + self.vibration * shape;
            let y = self.pos.y + y_offset;
            let current = vec2(x, y);

            draw_line(prev.x, prev.y, current.x, current.y, thickness, color);
            prev = current;
        }

        draw_circle(start.x, start.y - 10.0, 4.0, WHITE);

        draw_text(
            &format!("{:.1}Hz", self.frequency),
            start.x - 20.0,
            end.y + 20.0,
            16.0,
            LIGHTGRAY,
        );
    }
}
