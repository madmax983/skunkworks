...
async fn main() {
    ...
    loop {
        // Handle Input (Camera & Simulation)
        ...
        if is_mouse_button_pressed(MouseButton::Left) {
            ...
            if dist > 10.0 {
                let v_circ = (G * 5000.0 / dist as f64).sqrt();
        ...
        if !paused {
            let dt = 0.05;
            let physics_steps = 4;
            let physics_dt = dt / physics_steps as f64;
        ...
        if let Ok(mut state) = audio_state.write() {
            ...
            for (i, body) in universe.bodies.iter().enumerate() {
                ...
                let freq = 50.0 + speed as f32 * 10.0;
                let vol = (1000.0 / (dist as f32 + 100.0)).clamp(0.0, 0.5);
