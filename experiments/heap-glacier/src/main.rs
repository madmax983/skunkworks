...
async fn main() {
    ...
    loop {
        ...
        if is_key_down(KeyCode::Left) { yaw -= 0.05; }
        if is_key_down(KeyCode::Right) { yaw += 0.05; }
        if is_key_down(KeyCode::Up) { pitch += 0.05; }
        if is_key_down(KeyCode::Down) { pitch -= 0.05; }

        if is_key_pressed(KeyCode::Key1) { auto_mode = 1; }
        if is_key_pressed(KeyCode::Key2) { auto_mode = 2; }
        if is_key_pressed(KeyCode::Key3) { auto_mode = 3; }
        if is_key_pressed(KeyCode::Key0) { auto_mode = 0; }
        ...
        set_camera(&Camera3D {
            position,
            up: vec3(0.0, 1.0, 0.0),
            target: position + vec3(yaw.sin() * pitch.cos(), pitch.sin(), yaw.cos() * pitch.cos()),
            ..Default::default()
        });
        ...
        for y in 0..height-1 {
            for x in 0..width-1 {
                ...
                if w00 > 0.01 || w10 > 0.01 || w01 > 0.01 || w11 > 0.01 {
                    ...
                    let b00 = h00 + i00; let b10 = h10 + i10;
                    let b01 = h01 + i01; let b11 = h11 + i11;
        ...
        draw_text(format!("Mode: {} (0:None, 1:Web, 2:Leak, 3:GC)", auto_mode).as_str(), 20.0, 60.0, 20.0, BLACK);
