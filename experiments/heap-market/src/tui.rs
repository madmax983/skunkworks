...
fn draw_heap_map(f: &mut Frame, heap: &Heap, area: Rect) {
    let block_widget = Block::default().borders(Borders::ALL).title(format!(
        " Heap Map (Capacity: {}, Blocks: {}) ",
        heap.capacity,
        heap.blocks.len()
    ));
...
fn draw_ticker(f: &mut Frame, market: &Market, area: Rect) {
    let history: Vec<(f64, f64)> = market
        .history
        .iter()
        .enumerate()
        .map(|(i, &p)| (i as f64, p))
