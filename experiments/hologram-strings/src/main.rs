...
impl App {
    fn new() -> Self {
        ...
        for _ in 0..64 {
            strings.push(StringPhysics::new(rng.gen_range(10.0..100.0)));
        }
    ...
    fn update_hologram(&mut self) {
        let mut grid = vec![0.0; 256 * 128];
        for (i, s) in self.strings.iter().enumerate() {
            let x = 10 + i * 3;
            if x < 256 {
                // String vibration mapped to vertical line
                let v = s.vibration.abs() as f64;
                for y in 20..108 {
                    grid[y * 256 + x] = v;
                }
            }
        }
        self.hologram = Hologram::from_grid(&grid, 256, 128);
            ...
            if event::poll(Duration::from_millis(50))? {
                ...
                for s in &mut self.strings {
                    ...
                    if rng.gen_bool(0.01) {
                        s.pluck(rng.gen_range(10.0..50.0));
                    }
                }
