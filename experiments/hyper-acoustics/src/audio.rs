...
use crate::grid::{PhysicsGrid4D, GRID_SIZE};

#[cfg(feature = "audio")]
use cpal::traits::{DeviceTrait, HostTrait, StreamTrait};

pub enum AudioCommand {
    Pluck { x: usize, y: usize, z: usize, w: usize, strength: f32 },
    SetParams { c2: f32, damping: f32 },
}
            ...
            move |data: &mut [f32], _: &cpal::OutputCallbackInfo| {
                for sample in data.iter_mut() {
                    ...
                    while let Ok(cmd) = cmd_rx.try_recv() {
                        match cmd {
                            AudioCommand::Pluck { x, y, z, w, strength } => {
                                grid.pluck(x, y, z, w, strength);
                            },
                            AudioCommand::SetParams { c2: new_c2, damping: new_damping } => {
                                c2 = new_c2;
                                damping = new_damping;
                            }
                        }
    ...
    #[cfg(not(feature = "audio"))]
    {
        ...
        let thread = std::thread::spawn(move || {
            ...
            loop {
                ...
                next_tick += duration;
                if let Some(sleep_time) = next_tick.checked_duration_since(std::time::Instant::now()) {
