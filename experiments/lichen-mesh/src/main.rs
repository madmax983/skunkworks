...
impl Grid {
    fn new(width: usize, height: usize) -> Self {
        ...
        let mut cells = vec![Cell { cell_type: CellType::Empty, energy: 0.0, water: 0.0, age: 0, active: false }; width * height];
    ...
    fn update(&mut self) {
            ...
            for x in 0..self.width {
                ...
                match cell.cell_type {
                    CellType::Algae => {
                        ...
                        let fungus_neighbors: Vec<usize> = neighbors.iter()
                    ...
                    CellType::Fungus => {
                        ...
                        let algae_neighbors: Vec<usize> = neighbors.iter()
        ...
        for i in 0..self.cells.len() {
            if !next_cells[i].active { continue; }
            ...
            if next_cells[i].energy <= 0.1 || next_cells[i].water <= 0.1 {
                 if rng.gen::<f32>() < 0.05 {
                     next_cells[i] = Cell { cell_type: CellType::Empty, active: false, ..next_cells[i] };
                 }
            }
        }
        ...
        for y in 0..self.height {
            for x in 0..self.width {
                ...
                let neighbors = self.get_neighbors(x, y);
                let empty_neighbors: Vec<usize> = neighbors.iter()
                    .filter(|&&ni| !current_cells_state[ni].active)
                    .cloned()
                    .collect();

                if empty_neighbors.is_empty() { continue; }

                if cell.cell_type == CellType::Algae && cell.energy > 15.0 && cell.water > 5.0 {
                     if let Some(&target) = empty_neighbors.get(rng.gen_range(0..empty_neighbors.len())) {
                         let new_type = if rng.gen::<f32>() < 0.3 { CellType::Fungus } else { CellType::Algae };
                         next_cells[target] = Cell::new(new_type);
                         next_cells[idx].energy -= 10.0;
                     }
                } else if cell.cell_type == CellType::Fungus && cell.water > 15.0 && cell.energy > 5.0 {
                     if let Some(&target) = empty_neighbors.get(rng.gen_range(0..empty_neighbors.len())) {
                         let new_type = if rng.gen::<f32>() < 0.3 { CellType::Algae } else { CellType::Fungus };
                         next_cells[target] = Cell::new(new_type);
...
async fn main() {
    ...
    loop {
                ...
                if cell.active {
                    let (color, size_factor) = match cell.cell_type {
                        CellType::Algae => {
                            ...
                            (Color::new(0.0, brightness, 0.2 + brightness * 0.1, 1.0), 0.9)
                        },
                        CellType::Fungus => {
        ...
        // Overlay UI
        draw_text("LICHEN MESH // SYMBIOTIC MICROSERVICES", 10.0, 30.0, 30.0, WHITE);

        let algae_count = grid.cells.iter().filter(|c| c.active && c.cell_type == CellType::Algae).count();
        let fungus_count = grid.cells.iter().filter(|c| c.active && c.cell_type == CellType::Fungus).count();

        draw_text(&format!("FRONTEND NODES: {}", algae_count), 10.0, 60.0, 20.0, GREEN);
        draw_text(&format!("BACKEND NODES: {}", fungus_count), 10.0, 80.0, 20.0, LIGHTGRAY);
        draw_text(&format!("PACKETS: {}", grid.packets.len()), 10.0, 100.0, 20.0, YELLOW);
