use crate::history::{FileMapper, GeologicalEvent, HistoryStream};
...
#[macroquad::main("Lithosphere Git")]
async fn main() {
    ...
    loop {
        ...
        if !paused {
            for _ in 0..speed {
                        ...
                        for event in events {
                            match event {
                                ...
                                GeologicalEvent::Weathering { x, y, intensity } => {
                                    ...
                                    let iters = (intensity * 50.0) as usize;
                                    for _ in 0..iters {
                ...
                // Always erode a bit globally to simulate time passing
                terrain.erode(20);
            }
        }
        ...
        // UI
        draw_text(
            &format!("Commit: {} {}", current_hash, current_msg),
            10.0,
            30.0,
            20.0,
            BLACK,
        ...
        draw_text(
            &format!("Speed: {}x | Paused: {}", speed, paused),
            10.0,
            50.0,
            20.0,
            BLACK,
        ...
        draw_text(
            "Controls: Arrows/PgUp/PgDn to move, Space to pause, +/- speed",
            10.0,
            70.0,
            20.0,
            DARKGRAY,
...
fn draw_terrain(terrain: &Terrain) {
    ...
    for z in 0..terrain.height - 1 {
        for x in 0..terrain.width - 1 {
            ...
            let sed = terrain.sediment[terrain.get_index(x, z)];
...
fn get_color(height: f32, sediment: f32) -> Color {
    // Height coloring
    // let _h_norm = (height / 20.0).clamp(0.0, 1.0); // unused

    if sediment > 0.2 {
        return Color::new(0.8, 0.7, 0.4, 1.0); // Sand
    }
