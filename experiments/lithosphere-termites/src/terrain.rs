...
impl Terrain {
    ...
    pub fn take_sediment(&mut self, x: usize, y: usize, amount: f32) -> f32 {
        ...
        // But for simplicity, let's just take from height if sediment is low?
        // Or termites strictly move sediment.
        // Let's say termites can mine rock (height) and turn it into sediment.

        let available = self.heightmap[idx];
        let taken = amount.min(available);
        self.heightmap[idx] -= taken;

        // Also reduce sediment visual if any
        if self.sediment[idx] > 0.0 {
            self.sediment[idx] = (self.sediment[idx] - taken).max(0.0);
        }
    ...
    fn erode_droplet(&mut self, mut x: f32, mut y: f32) {
        ...
        for _ in 0..max_lifetime {
            ...
            if sediment > capacity || diff > 0.0 {
                ...
                sediment -= amount;
                self.deposit(x - dir_x, y - dir_y, amount);
            } else {
                // Erode
            ...
            let new_speed_sq = speed * speed - diff * gravity;
            if new_speed_sq < 0.0 {
                speed = 0.0;
            } else {
                speed = new_speed_sq.sqrt();
            }
