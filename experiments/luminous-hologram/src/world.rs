...
impl World {
    ...
    pub fn update(&mut self, attractors: &[(f64, f64)]) {
        ...
        for (i, boid) in self.boids.iter().enumerate() {
            ...
            // 1b. Calculate Attractor Force (towards nearest target point from hologram)
            if !attractors.is_empty() {
                let mut nearest_dist_sq = f64::MAX;
                let mut nearest_target = Vec2::new(0.0, 0.0);

                for &(ax, ay) in attractors {
                    let target = Vec2::new(ax, ay);
                    let dist_sq = boid.position.distance_squared(target);
                ...
                // Seek nearest
                let desired = (nearest_target - boid.position).normalize() * boid.dna.max_speed;
                let steer = (desired - boid.velocity).limit(boid.dna.max_force);

                // Mix attractor force with flocking. If it's close, let it settle.
                let distance = nearest_dist_sq.sqrt();
                let arrive_factor = if distance < 5.0 { distance / 5.0 } else { 1.0 };

                flocking_force += steer * 1.5 * arrive_factor;
            }
