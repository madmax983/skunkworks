...
async fn async_main() {
    ...
    loop {
        ...
        // UI
        let payload_text = format!("Payload: {}", payload);
        draw_text(&payload_text, 20.0, 30.0, 30.0, WHITE);
        draw_text(
...
fn jewel_color_to_macroquad(color: JewelColor) -> Color {
    match color {
        JewelColor::Red => RED,
...
fn draw_mandala(mandala: &Mandala, global_rotation: f32) {
    ...
    for symmetry_idx in 0..mandala.config.symmetry_order {
        ...
        for (idx, jewel_opt) in mandala.jewels.iter().enumerate() {
            let Some(jewel) = jewel_opt else { continue };
            ...
            let color = jewel_color_to_macroquad(jewel.color);
