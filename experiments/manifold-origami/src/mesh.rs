...
impl Mesh {
    ...
    pub fn generate_miura_ori(&mut self, system: &mut PbdSystem, rows: usize, cols: usize) {
        self.rows = rows;
        self.cols = cols;
        self.indices.clear();
        ...
        for i in 0..=rows {
            for j in 0..=cols {
                let x = j as f32 * a;
                // Zig-zag offset for odd columns? Or odd rows?
                // Standard Miura:
                // Vertices are at (j*a, i*b + (j%2)*s)
                ...
                // Initial bias for folding (M/V)
                // We want the pattern to be:
                // Verticals (j const): Alternating M/V.
