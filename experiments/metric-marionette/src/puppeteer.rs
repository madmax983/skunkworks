use crate::skeleton::{Skeleton, Joint};
...
impl Puppeteer {
    ...
    pub fn build_skeleton() -> Skeleton {
        ...
        let head = Joint { name: "head".into(), length: 4.0, angle: 0.0, children: vec![] }; // Relative to neck
        let neck = Joint { name: "neck".into(), length: 2.0, angle: 0.0, children: vec![head] }; // Relative to spine

        // Arms (Starting from shoulder)
        // Left Arm (Points Left: PI). Parent Spine is Up (PI/2). Relative: PI/2.
        let l_hand = Joint { name: "l_hand".into(), length: 3.0, angle: 0.0, children: vec![] };
        let l_forearm = Joint { name: "l_forearm".into(), length: 6.0, angle: 0.0, children: vec![l_hand] };
        let l_arm = Joint { name: "l_arm".into(), length: 6.0, angle: PI/2.0, children: vec![l_forearm] };

        // Right Arm (Points Right: 0). Parent Spine is Up (PI/2). Relative: -PI/2.
        let r_hand = Joint { name: "r_hand".into(), length: 3.0, angle: 0.0, children: vec![] };
        let r_forearm = Joint { name: "r_forearm".into(), length: 6.0, angle: 0.0, children: vec![r_hand] };
        let r_arm = Joint { name: "r_arm".into(), length: 6.0, angle: -PI/2.0, children: vec![r_forearm] };
        ...
        let l_foot = Joint { name: "l_foot".into(), length: 2.0, angle: PI/2.0, children: vec![] };
        let l_shin = Joint { name: "l_shin".into(), length: 8.0, angle: 0.0, children: vec![l_foot] };
        let l_thigh = Joint { name: "l_thigh".into(), length: 8.0, angle: -PI/2.0 - 0.3, children: vec![l_shin] };

        // Right Leg (Down-Right)
        let r_foot = Joint { name: "r_foot".into(), length: 2.0, angle: PI/2.0, children: vec![] };
        let r_shin = Joint { name: "r_shin".into(), length: 8.0, angle: 0.0, children: vec![r_foot] };
        let r_thigh = Joint { name: "r_thigh".into(), length: 8.0, angle: -PI/2.0 + 0.3, children: vec![r_shin] };
