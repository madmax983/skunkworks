...
//! **Lineage**: A hybrid of `mnem-rot` (Entropy-driven codebase decay) crossed with `ferrous-fluid` (Magnetic particles).
//!
//! The codebase's health actively repulses or attracts a swarm of magnetic particles. Rotting code pushes particles away, while healthy, freshly maintained code attracts them, acting like a physical magnetic medium representing codebase entropy.
//!
...
async fn main() {
    ...
    loop {
        ...
        // Update fluid particles based on nodes
        for p in particles.iter_mut() {
            let mut p_force = vec2(0.0, 0.0);

            for node in graph.nodes.iter() {
                let diff = node.pos - p.pos;
                let dist_sq = diff.length_squared().max(1.0);
                if dist_sq < 90000.0 {
                    // Healthy nodes attract (+ force), rotting nodes repulse (- force)
                    let force_mag = (node.health - 0.5) * 5000.0 / dist_sq.sqrt();
                    p_force += diff.normalize() * force_mag;
                }
            }

            p.vel += p_force * dt;
            p.vel *= 0.95; // Fluid damping
            p.pos += p.vel;
