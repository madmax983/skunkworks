...
impl Agent {
    ...
    pub fn update(&mut self, world: &World) -> AgentUpdateResult {
        ...
        let sensor_dist = 9.0;
        let turn_angle = PI / 4.0; // Sharp turns for tracing data
        let speed = 1.0;
        ...
        AgentUpdateResult {
            deposit_x: ix,
            deposit_y: iy,
            deposit_amount: 5.0, // Strong deposit
        }
    }
}
...
impl World {
    ...
    pub fn from_image(img: &RgbaImage, num_agents: usize) -> (Self, Vec<Agent>) {
        let width = img.width() as usize;
        let height = img.height() as usize;
        let mut world = Self::new(width, height);

        // Initialize trails based on LSBs
        for (x, y, pixel) in img.enumerate_pixels() {
            let mut val = 0.0;
            // Check LSB of any channel
            if (pixel[0] & 1) == 1 || (pixel[1] & 1) == 1 || (pixel[2] & 1) == 1 {
                val = 255.0; // Food!
            }
            world.set_trail(x as usize, y as usize, val);
        }
