...
impl World {
    ...
    pub fn update(&mut self) -> Vec<f32> {
        ...
        for i in 0..count {
            ...
            // Base input current (random noise) to keep brain alive
            if rng.gen::<f64>() < 0.05 {
                synaptic_input += 15.0;
            }
            ...
            for j in 0..count {
                ...
                // --- Neural Logic ---
                // If neighbor spiked last frame and is close enough -> Synaptic input
                if d_sq < coupling_radius_sq && b2.spiked {
                    // Strength decreases with distance? Or constant?
            ...
            input_currents[i] = synaptic_input;
        }
        ...
        for (i, boid) in self.boids.iter_mut().enumerate() {
            ...
            // Apply neuron update
            // Using 1.0 time step for convenience
            let spiked = boid.neuron.update(1.0, input_currents[i]);
            boid.spiked = spiked;

            if spiked {
                boid.spike_timer = 5;
                spikes.push(boid.dna.base_freq);
            }
        }
