    ...
    pub fn new(width: usize, height: usize, swarm_size: usize) -> Self {
        ...
        for _ in 0..swarm_size {
            boids.push(Boid {
                ...
                velocity: Vec2::new(rand::random::<f64>() - 0.5, rand::random::<f64>() - 0.5)
                    .normalize()
                    * 10.0,
            });
        }
    ...
    pub fn update(&mut self, dt: f64) {
        ...
        for i in 0..self.boids.len() {
            ...
            if self.boids[i].velocity.magnitude() > flocking_params.max_speed {
                self.boids[i].velocity =
                    self.boids[i].velocity.normalize() * flocking_params.max_speed;
            }
            let vel = self.boids[i].velocity;
            self.boids[i].position += vel * dt;

            // simple bounds
            let half_w = (self.grid_width as f64 * 10.0) / 2.0;
            let half_h = (self.grid_height as f64 * 10.0) / 2.0;

            if self.boids[i].position.x > half_w {
                self.boids[i].position.x = -half_w;
            }
            if self.boids[i].position.x < -half_w {
                self.boids[i].position.x = half_w;
            }
            if self.boids[i].position.y > half_h {
                self.boids[i].position.y = -half_h;
            }
            if self.boids[i].position.y < -half_h {
                self.boids[i].position.y = half_h;
            }
        ...
        for boid in &self.boids {
            ...
            let x_grid = (x_norm * (self.grid_width - 1) as f64)
                .clamp(0.0, (self.grid_width - 1) as f64) as usize;
            let y_grid = (y_norm * (self.grid_height - 1) as f64)
