...
async fn main() {
    ...
    for i in 0..NUM_BOIDS {
        ...
        mq_velocities[i] = vec2(rand::gen_range(-1.0, 1.0), rand::gen_range(-1.0, 1.0))
            .normalize_or_zero()
            * FLOCKING_PARAMS.max_speed as f32;
    ...
    loop {
        ...
        for i in 0..NUM_BOIDS {
            ...
            if speed > FLOCKING_PARAMS.max_speed as f32 {
                new_mq_velocities[i] =
                    (new_mq_velocities[i] / speed) * FLOCKING_PARAMS.max_speed as f32;
            }

            mq_positions[i] += new_mq_velocities[i] * dt;

            // Simple wrap-around
            if mq_positions[i].x > 200.0 {
                mq_positions[i].x = -200.0;
            }
            if mq_positions[i].x < -200.0 {
                mq_positions[i].x = 200.0;
            }
            if mq_positions[i].y > 200.0 {
                mq_positions[i].y = -200.0;
            }
            if mq_positions[i].y < -200.0 {
                mq_positions[i].y = 200.0;
            }
            ...
            flock_positions[i] = LocusVec2::new(mq_positions[i].x as f64, mq_positions[i].y as f64);
            flock_velocities[i] =
                LocusVec2::new(new_mq_velocities[i].x as f64, new_mq_velocities[i].y as f64);
        }
        ...
        for i in 0..mesh_data.vertices.len() {
            ...
            for j in 0..NUM_BOIDS {
                ...
                if dist_sq < effect_radius * effect_radius {
                    influence += (1.0 - (dist_sq.sqrt() / effect_radius)) * 20.0;
                    // Raise vertices near boids
                }
            }
