...
impl Phoneme {
    ...
    pub fn from_char(c: char) -> Option<Self> {
        match c.to_ascii_lowercase() {
            ...
            _ => None, // Punctuation/Spaces return None
        }
    }
...
mod tests {
    ...
    fn test_phoneme_properties() {
        ...
        assert!(p.is_consonant());
        assert!(!p.is_vowel());

        let a = Phoneme::from_char('a').unwrap();
        assert!(a.is_vowel());
    }

    #[test]
    fn test_grimms_law_change() {
        // Force random seed or just loop until change for probabilistic test
        let mut rng = rand::thread_rng();
        let rule = Rule::GrimmsLaw;

        // Pater -> Father (p->f, t->s)
        // Try many times until it changes
        let mut changed = false;
        for _ in 0..100 {
            let mut w = Word::new("pater");
            if rule.apply(&mut w, &mut rng) {
                // Check if changed correctly
                // p -> f
                if w.phonemes[0].symbol == 'f' || w.phonemes[2].symbol == 's' {
                    changed = true;
                    break;
                }
            }
        }
        assert!(changed, "Grimms Law should eventually apply");
    }
}
