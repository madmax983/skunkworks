...
fn ui(f: &mut Frame, sim: &Simulation) {
    ...
    let rows: Vec<Row> = sim
        .processes
        .iter()
        .map(|p| {
            let state_str = match p.state {
                ProcessState::Ready => "Ready",
                ProcessState::Running(c) => {
                    return Row::new(vec![
    ...
    let list = List::new(items).block(Block::default().title("CPU Cores").borders(Borders::ALL));
