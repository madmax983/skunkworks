...
pub fn add_qubit(
    next_system_id: &mut usize,
    systems: &mut HashMap<usize, QubitSystem>,
    entity_map: &mut HashMap<usize, (usize, usize)>,
    entity_id: usize,
    initial_state_one: bool,
) {
    let sys_id = *next_system_id;
    *next_system_id += 1;
    ...
    systems.insert(sys_id, sys);
    entity_map.insert(entity_id, (sys_id, 0));
}

pub fn apply_gate(
    systems: &mut HashMap<usize, QubitSystem>,
    entity_map: &HashMap<usize, (usize, usize)>,
    gate: Gate,
    entity_id: usize,
) -> Result<()> {
    if let Some(&(sys_id, qubit_idx)) = entity_map.get(&entity_id) {
        if let Some(sys) = systems.get_mut(&sys_id) {
...
pub fn entangle(
    ...
    let (sys1_id, idx1) = *entity_map.get(&id1).ok_or(anyhow!("Entity 1 not found"))?;
    let (sys2_id, idx2) = *entity_map.get(&id2).ok_or(anyhow!("Entity 2 not found"))?;

    if sys1_id == sys2_id {
        // Already in same system, just apply CNOT
        if let Some(sys) = systems.get_mut(&sys1_id) {
        ...
        *next_system_id += 1;
        ...
        for val in entity_map.values_mut() {
            if val.0 == sys1_id {
                val.0 = new_sys_id;
                // val.1 stays same
        ...
        systems.insert(new_sys_id, merged);
        ...
        if let Some(sys) = systems.get_mut(&new_sys_id) {
...
pub fn measure(
    ...
    dissolve_system(next_system_id, systems, entity_map, sys_id)?;
...
pub fn dissolve_system(
    ...
    let sys = systems.remove(&sys_id).ok_or(anyhow!("System not found"))?;
    ...
    for (eid, (sid, qid)) in entity_map.iter() {
        if *sid == sys_id {
    ...
    for (eid, qid) in entities_in_sys {
        ...
        *next_system_id += 1;
        ...
        systems.insert(new_sys_id, new_sys);
        entity_map.insert(eid, (new_sys_id, 0));
    }
...
pub fn get_probability(
    systems: &HashMap<usize, QubitSystem>,
    entity_map: &HashMap<usize, (usize, usize)>,
    entity_id: usize,
) -> f64 {
    if let Some(&(sys_id, qubit_idx)) = entity_map.get(&entity_id) {
        if let Some(sys) = systems.get(&sys_id) {
