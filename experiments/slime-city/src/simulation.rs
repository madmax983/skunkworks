...
impl World {
    ...
    pub fn with_cities_and_agents(
        ...
        let agents_per_city = total_agents_target / num_cities;
    ...
    pub fn update_agents_parallel(&mut self, agents: &mut [Agent]) {
        ...
        // Single threaded write to grid is safer/easier than atomic floats or mutex grid
        // Given 1M agents, this serial part might be a bottleneck.
        // Optimization: Use `dashmap` or partition the grid?
        // For now, let's stick to serial write as "deposit" is just adding to a coord.
        // Actually, we can check performance later.
    ...
    pub fn diffuse_and_decay(&mut self) {
        let decay_factor = 0.95; // Slower decay for persistent trails
