...
use crate::mechanism::{Pin, LogicLever};
...
fn instruction_trigger_system(
    ...
    for event in collision_events.read() {
        if let CollisionEvent::Started(e1, e2, _) = event {
            ...
            let pin_ent = if pin_query.contains(*e1) { *e1 } else if pin_query.contains(*e2) { *e2 } else { continue };
            let lever_ent = if lever_query.contains(*e1) { *e1 } else if lever_query.contains(*e2) { *e2 } else { continue };
            ...
            if let Ok(pin) = pin_query.get(pin_ent) {
                if let Ok(_lever) = lever_query.get(lever_ent) {
                    ...
                    let instr_str = match pin.note_index {
                        0 => { cpu_state.accumulator += 1; "INC" },
                        1 => { cpu_state.accumulator -= 1; "DEC" },
                        2 => { cpu_state.accumulator += 5; "ADD 5" },
                        3 => { cpu_state.accumulator -= 2; "SUB 2" },
                        4 => { cpu_state.accumulator = 0; "CLR" },
                        _ => "NOP",
                    };

                    cpu_state.pc += 1;
                    let pc = cpu_state.pc;
                    cpu_state.instruction_history.push(format!("{}: {}", pc, instr_str));
...
fn update_ui(
    cpu_state: Res<CpuState>,
    mut query: Query<&mut Text, With<CpuDisplay>>,
) {
