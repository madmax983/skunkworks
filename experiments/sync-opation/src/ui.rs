...
use crate::conductor::{UiEvent, MusicianState};
    ...
    fn run_loop(&mut self, terminal: &mut Terminal<CrosstermBackend<std::io::Stdout>>) -> Result<()> {
        loop {
            ...
            terminal.draw(|f| {
                ...
                let title = Paragraph::new("Sync-Opation: Thread Contention Rhythms (Press 'q' to quit)")
                ...
                let rows: Vec<Row> = sorted_states.iter().map(|(id, state)| {
                ...
                let table = Table::new(rows, widths)
                    .header(Row::new(vec!["ID", "State"]).style(Style::default().fg(Color::White).bg(Color::Black)))
