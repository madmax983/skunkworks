...
// 👺 Havoc: Prove that `SystemStats` update thread can be starved!
// By having reader threads aggressively lock the mutex, the sysinfo updater
// thread can be blocked indefinitely from writing new stats.
#[test]
fn havoc_test_monitor_starvation() {
    let status = std::process::Command::new(std::env::current_exe().unwrap())
        .arg("--exact")
        .arg("havoc_test_monitor_starvation_inner")
        .arg("--nocapture")
        .arg("--ignored")
        .status()
...
#[test]
#[ignore]
fn havoc_test_monitor_starvation_inner() {
    if std::env::args().any(|arg| arg == "havoc_test_monitor_starvation_inner") {
        ...
        // Writer thread (simulating the sysinfo loop)
