...
impl Fluid {
    ...
    pub fn step(&mut self) {
        ...
        // Project
        project(&mut self.vx0, &mut self.vy0, &mut self.vx, &mut self.vy, iter, n);
        ...
        // Project
        project(&mut self.vx, &mut self.vy, &mut self.vx0, &mut self.vy0, iter, n);

        // Diffuse density
        diffuse(0, &mut self.s, &self.density, self.diff, self.dt, iter, n);

        // Advect density
        advect(0, &mut self.density, &self.s, &self.vx, &self.vy, self.dt, n);
    }
}
...
fn set_bnd(b: usize, x: &mut [f32], n: usize) {
    for i in 1..n-1 {
        ...
        x[(n - 1) + i * n] = if b == 1 { -x[(n - 2) + i * n] } else { x[(n - 2) + i * n] };
    }
    for i in 1..n-1 {
...
fn diffuse(b: usize, x: &mut [f32], x0: &[f32], diff: f32, dt: f32, iter: usize, n: usize) {
    ...
    for _ in 0..iter {
        for j in 1..n-1 {
            for i in 1..n-1 {
                let idx = i + j * n;
                x[idx] = (x0[idx] + a * (x[i-1 + j*n] + x[i+1 + j*n] + x[i + (j-1)*n] + x[i + (j+1)*n])) / (1.0 + 4.0 * a);
            }
        }
        set_bnd(b, x, n);
    }
...
fn project(u: &mut [f32], v: &mut [f32], p: &mut [f32], div: &mut [f32], iter: usize, n: usize) {
    for j in 1..n-1 {
        for i in 1..n-1 {
            div[i + j*n] = -0.5 * (u[i+1 + j*n] - u[i-1 + j*n] + v[i + (j+1)*n] - v[i + (j-1)*n]) / (n as f32);
            p[i + j*n] = 0.0;
        }
    }
    set_bnd(0, div, n);
    ...
    for _ in 0..iter {
        for j in 1..n-1 {
            for i in 1..n-1 {
                p[i + j*n] = (div[i + j*n] + p[i-1 + j*n] + p[i+1 + j*n] + p[i + (j-1)*n] + p[i + (j+1)*n]) / 4.0;
            }
        }
        set_bnd(0, p, n);
    }
...
mod tests {
    ...
    fn test_diffusion() {
        ...
        println!("Center: {}, Right: {}", fluid.density[idx_center], fluid.density[idx_right]);
    ...
    fn test_advection() {
        ...
        // Let's just check that density changed location.
         ...
         assert!(fluid.density[idx_right] > 0.0 || fluid.density[idx_center] < 100.0, "Density did not move!");
    }
}
