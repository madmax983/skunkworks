...
pub fn draw_ui(f: &mut Frame, state: &TuiState) {
    ...
    let header = Paragraph::new(format!(
        "THREAD FREQUENCY :: Time: {:.2}s :: Sample: {}",
        time_sec, now
    ))
    .style(
        Style::default()
    ...
    for (i, name) in state.musician_names.iter().enumerate() {
        ...
        let time_diff = if now >= last_beat {
            now - last_beat
        } else {
            last_beat - now
        }; // Absolute diff
        ...
        let color = if is_clash { Color::Red } else { Color::Green };
        ...
        let label = format!(
            "{} ({}ms) - {:?}",
            name, state.musician_periods[i], state.musician_voices[i]
        );
