...
impl VocalTract {
    ...
    pub fn update_area(&mut self, index: usize, area: f32) {
        if index < self.areas.len() {
            // Clamp area to non-zero to avoid div by zero
            self.areas[index] = area.max(0.001);
    ...
    T: cpal::Sample + cpal::FromSample<f32>,
{
    ...
    let stream = device.build_output_stream(
        ...
        move |data: &mut [T], _: &cpal::OutputCallbackInfo| {
            ...
            // Update tenseness from params? Maybe add it later.
...
mod tests {
    ...
    fn test_glottal_source_output() {
        ...
        let mut max_amp: f32 = 0.0;
        // Sample at 44100Hz for enough samples to cover a cycle
        for _ in 0..1000 {
    ...
    fn test_tract_propagation() {
        // Create a tract of length 10
        let mut tract = VocalTract::new(10, 1.0);

        // Input an impulse
        tract.process(1.0);

        // Check for output after some delay (travel time back and forth)
        let mut received_energy = false;
        // Should take ~10 steps to reach end
        for _ in 0..50 {
