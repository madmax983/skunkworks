...
async fn main() {
    ...
    let root = pendulum.add_node(
        Vec2::new(WORLD_SIZE / 2.0, WORLD_SIZE / 2.0),
        1.0,
        true,
        "Root".to_string(),
    );
    let joint1 = pendulum.add_node(
        Vec2::new(WORLD_SIZE / 2.0 + 100.0, WORLD_SIZE / 2.0),
        10.0,
        false,
        "Joint1".to_string(),
    );
    let joint2 = pendulum.add_node(
        Vec2::new(WORLD_SIZE / 2.0 + 200.0, WORLD_SIZE / 2.0),
        10.0,
        false,
        "Joint2".to_string(),
    );
    ...
    pendulum.nodes[joint2].prev_pos +=
    ...
    loop {
        ...
        for node in &pendulum.nodes {
            ...
            draw_circle(
                node.pos.x * screen_scale_x,
                node.pos.y * screen_scale_y,
                radius,
                color,
            );
        }
        ...
        draw_text(
            &format!(
                "Server Health: {:.1}%",
                (world.server_health / world.max_health) * 100.0
            ),
            20.0,
            60.0,
            20.0,
            if world.server_health > 0.0 {
                GREEN
            } else {
                RED
            },
