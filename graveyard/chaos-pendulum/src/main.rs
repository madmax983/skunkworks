...
async fn main() {
    ...
    loop {
        ...
        if is_key_down(KeyCode::Up) {
            ...
            for g in &mut ghosts {
                g.gravity.y = real_system.gravity.y;
            }
        }
        ...
        if is_key_down(KeyCode::Right) {
            // Less friction (higher value)
            real_system.friction = (real_system.friction + 0.001).min(1.0);
            for g in &mut ghosts {
                g.friction = real_system.friction;
            }
        }
        if is_key_down(KeyCode::Left) {
            // More friction (lower value)
        ...
        let world_mouse =
            (mouse_vec - Vec2::new(screen_width() / 2.0, screen_height() / 2.0)) / zoom + target;
