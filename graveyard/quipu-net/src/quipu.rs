...
mod tests {
    ...
    fn test_knot_creation() {
        let mut p = Pendant::new([0, 0, 0]);
        p.add_number(123);
        // 123 -> 1 hundred, 2 tens, 3 units
        // Order of addition: 3 (units), 2 (tens), 1 (hundreds)
        // My implementation adds digits from highest power (100) down to lowest (1).
        // Wait, let's check the loop in `add_number`.
        // "for (p, &digit) in digits.iter().enumerate().rev()"
        // digits for 123 are [3, 2, 1] (because push(value % 10))
        // enumerate: [(0, 3), (1, 2), (2, 1)]
        // rev(): [(2, 1), (1, 2), (0, 3)]
        //
        // Loop 1: p=2 (hundreds), digit=1.
        // Adds 1 Single knot. Power=2.
        // Loop 2: p=1 (tens), digit=2.
        // Adds 2 Single knots. Power=1.
        // Loop 3: p=0 (units), digit=3.
        // Adds 1 Long(3) knot. Power=0.

        assert_eq!(p.knots.len(), 4);
        assert_eq!(p.knots[0].knot_type, KnotType::Single);
        assert_eq!(p.knots[0].power, 2);

        assert_eq!(p.knots[3].knot_type, KnotType::Long(3));
        assert_eq!(p.knots[3].power, 0);
    }
}
