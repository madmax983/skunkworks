...
fn write_data(
    ...
    // We lock once per buffer. In a real real-time system, we'd use a lock-free structure or a triple buffer.
    // For this experiment, a Mutex is fine as long as the critical section is short.
    ...
    for frame in output.chunks_mut(channels) {
        ...
        for (id, voice) in voices.iter_mut() {
            ...
            // Envelope Logic (Simple Linear Interpolation)
            let attack_speed = 0.05; // Fast attack
            let release_speed = 0.005; // Slower release
